...
当前执行：12C-R0 UI smoke 与布局基线
当前分支：main，执行前仍需通过 git 命令确认
已完成：12C-R0-01 Qt/MSVC Fresh Build Lane
下一任务：12C-R0-02 UI Self-Test 与 Smoke 基线
```
...
12C 只做 Qt 工作台收口：fresh build、Profile、设置生成配置、统一预览和诊断布局。fresh build gate 已通过；12C-R1/R2 仍需等待 R0 smoke 和布局基线完成。
...
现在只执行 Task 12C-R0-02，不执行后续 Task。
```
