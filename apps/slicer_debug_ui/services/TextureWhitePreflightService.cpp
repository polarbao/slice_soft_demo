...
struct TextureWhitePreflightService::CallbackState
{
    QMutex mutex;
    QPointer<TextureWhitePreflightService> service;
};

struct TextureWhitePreflightService::CacheState
{
    QMutex mutex;
    QHash<QString, TextureWhiteAssetScanResult> cache;
    QHash<QString, std::shared_future<TextureWhiteAssetScanResult>> inflight;
    quint64 decodecount{0U};
};

bool TextureWhitePreflightResult::HasWarning() const
{
    return shouldwarn && !warningmessage.isEmpty();
}

TextureWhitePreflightService::TextureWhitePreflightService(QObject* parent)
    : QObject(parent),
...
TextureWhitePreflightService::~TextureWhitePreflightService()
{
    QMutexLocker lock(&m_callbackState->mutex);
    m_callbackState->service = nullptr;
}

quint64 TextureWhitePreflightService::RequestScan(
    const TextureWhitePreflightRequest& request)
{
    ++m_generation;
    ...
    auto* runnable = new FunctionRunnable(
        ...
        {
            TextureWhitePreflightResult result;
            ...
            for (const QString& texturePath : request.texturepaths)
            {
                ...
                TextureWhiteAssetScanResult asset =
                    ScanAsset(cacheState, texturePath);
...
void TextureWhitePreflightService::Cancel()
{
...
bool TextureWhitePreflightService::IsRunning() const
{
    return m_running;
}

quint64 TextureWhitePreflightService::Generation() const
{
    return m_generation;
}

TextureWhitePreflightCacheDiagnostics
TextureWhitePreflightService::CacheDiagnostics() const
{
    QMutexLocker lock(&m_cacheState->mutex);
    TextureWhitePreflightCacheDiagnostics diagnostics;
...
TextureWhiteAssetScanResult TextureWhitePreflightService::ScanAsset(
    const std::shared_ptr<CacheState>& cacheState,
    const QString& texturePath)
{
    TextureWhiteAssetScanResult result;
    ...
    std::shared_future<TextureWhiteAssetScanResult> sharedResult;
    std::shared_ptr<std::promise<TextureWhiteAssetScanResult>> promise;
    {
        ...
        else
        {
            promise = std::make_shared<
                std::promise<TextureWhiteAssetScanResult>>();
...
void TextureWhitePreflightService::OnWorkerCompleted(
    TextureWhitePreflightResult result)
{
