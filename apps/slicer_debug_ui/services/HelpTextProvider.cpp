...
const QVector<SettingHelpMetadata>& MetadataEntries()
{
    static const QVector<SettingHelpMetadata> entries{
        ...
        MakeMetadata(
            ...
            QStringLiteral("填充颜色表层之间的模型实体区域。白墨写 W 通道，光油写 V 通道；它不同于模型外部 S 通道支撑。"),
            {QStringLiteral("W"), QStringLiteral("V")},
            QStringLiteral("生产 Profile 默认白墨，可显式选择光油"),
            QStringLiteral("生产 Profile 必须启用且不允许空填充"),
            kMaterialDoc),
        MakeMetadata(
            QStringLiteral("materialPolicy.white.enabled"),
            QStringLiteral("白墨材料策略"),
            QStringLiteral("启用既有白墨材料策略；实际覆盖范围同时受模型填充与工艺 Profile 约束。"),
            {QStringLiteral("W")},
            QStringLiteral("由 Profile 决定"),
            QStringLiteral("生产可用；不得与模型填充语义相互替代或重复"),
            kMaterialDoc),
        MakeMetadata(
        ...
        MakeMetadata(
            ...
            QStringLiteral("调试预览输出"),
            QStringLiteral("按配置生成 preview PNG/PPM，供 UI 快速浏览；生产 RGBWSV TIFF 不依赖该输出。"),
            {QStringLiteral("调试预览"), QStringLiteral("保存耗时")},
            QStringLiteral("由 Profile 决定"),
            QStringLiteral("不改变生产通道；只能作为调试视图，不能替代 TIFF 真源"),
            kWorkbenchDoc),
        MakeMetadata(
            QStringLiteral("preview.interval"),
            QStringLiteral("预览保存间隔"),
            QStringLiteral("每隔指定层数保存一组调试预览；值越小，图片数量与保存耗时越高。"),
            {QStringLiteral("调试预览"), QStringLiteral("I/O 耗时")},
            QStringLiteral("10 层"),
