...
struct DemoOptions
{
    ...
    std::filesystem::path outputDir{"output/SurfaceShellRealModel"};
    double voxelMm{0.05};
    double shellMm{0.10};
...
DemoOptions ParseOptions(const int argc, char** argv)
{
    ...
    for (int i{1}; i < argc; ++i)
    {
        ...
        {
            std::cout
                << "surface_shell_real_model_demo --config <config.json> --output <dir> "
                << "--voxel-mm <value> --shell-mm <value> "
                << "--mesh-policy strict_closed|warn_and_attempt --max-transfer-mm <value>\n";
            std::exit(0);
        }
        else
        {
    ...
    const slicer_core::SurfaceShellRealModelResult& result)
{
    ...
    {
        WritePng(
            previewDir / LayerName(mode, layer),
            result.shell.width,
            result.shell.height,
...
int RunDemo(const DemoOptions& options)
{
    ...
    {
        std::cerr << "surface_shell_real_model_demo: " << result.errors.front() << '\n';
        return 2;
    }

    WritePreviewSet(options.outputDir / "preview", result);
    std::cout << "surface_shell_real_model_demo: generated output\n"
              << "  format: " << result.input_format << "\n"
              << "  triangles: " << result.adapted_mesh.topology.accepted_triangles << "\n"
              << "  boundaryEdges: " << result.adapted_mesh.topology.boundary_edges << "\n"
              ...
              << "  fallbackVoxels: " << result.transfer.stats.fallback_voxels << "\n"
...
int main(const int argc, char** argv)
{
    ...
    {
        std::cerr << "surface_shell_real_model_demo: " << error.what() << '\n';
        return 1;
    }
}
