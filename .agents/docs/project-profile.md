...
- Latest completed task: TIFF T-A-03 switches the default Writer to LibTIFF 4.7.1 on the dynamic `x64-windows` triplet. Main Release alignment/contract/equivalence, semantic Package/RIP strict, Qt Runtime deployment, Stage 14 package dependency/checksum, and the explicit handwritten legacy lane all pass; default compression remains `none`.
...
- Current task: 16C-06-MEMFLOW bounded streaming memory remediation is ACTIVE by explicit user authorization (2026-08-18). MF-00..03B3 are complete: decision/design/context sync, overflow-safe RasterMemoryBudget, Legacy owned-layer transfer, compact occupancy range/caller-owned single-layer materialization, range-derived pre-shape support materialization, bounded outer-boundary/unsupported discovery, and non-production InternalVoid/Shape/footprint replay evidence. MF-03B4 base/varnish/material final replay remains only partially prepared. The new planners, materializers and scanners are not connected to Production Service yet. Retained Dense remains the production route; bounded Dense streaming, the multi-instance layer barrier, sparse Tile/Span, and adaptive routing must pass separate zero-drift Gates in that order. Stage 16 S0 remains the production default; 16D-02-R1 explicit S3 relief-heightfield support remains unchanged. P3 and any default switch remain unauthorized. Preserve SPI v1/11 exports/15 capabilities, Worker file contract, `p0.rgbwsv.2`, S2 path D and ViewData contracts.
...
- TIFF writer state: LibTIFF 4.7.1 is the default uint8 contiguous RGBWSV Writer and is deployed app-local with `tiff.dll` and its license. `slicesoft-handwritten-legacy` is an explicit legacy validation lane with a permanent known-failure alignment probe; standard stripped and 16-aligned tiled output use LibTIFF, while nonstandard tile fallback remains migration debt for T-A-05. `output.tiffCompression.algorithm=none|packbits` is propagated through both Writer lanes and production pipelines; manifests declare the effective algorithm and the strict Reader rejects tag/declaration mismatches. PackBits remains explicit, external target interoperability is pending, and default compression remains `none`.
...
S3 remains restricted to relief_heightfield; explicit textured relief and single-material W/V relief are allowed.
Single-material W/V S3 keeps texture disabled; ordinary untextured RGB + S3 fails closed.
