...
- Current task: Stage 16 is ACTIVE / PARTIAL GO. 16A-01..06, 16B-01..03, 16C-01..03 and 16D-01 are complete. S3 is integrated only as an explicit `relief_heightfield` candidate through Host, Worker, Scene Effective Config and Production Service; S0 remains default, while S2/S4/unknown values and contract mismatches fail closed. 16C-03 now performs one final support statistics scan after all support post-processing and exposes diagnostic-only `supportStatisticsScanCount`; three real-model TIFF/RIP output is unchanged. 16D-02 is dependency-ready. P3 contact leveling remains diagnostic-only pending separate 16B-04 authorization. Preserve SPI v1/11 exports/15 capabilities, Worker file contract, `p0.rgbwsv.2`, S2 path D and ViewData contracts.
