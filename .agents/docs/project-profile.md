...
- Current phase: `03D-01..06` are complete; the LibTIFF Release matrix concluded `GO_OPTIONAL`. `03D-07` optional-backend closure is READY while default switching is blocked. `12E-09D` remains the next prepared material-control specialty after 03D closure.
- Latest completed milestone: `03D-06 LibTIFF Release Writer performance matrix and GO_OPTIONAL decision`
...
- Current task: `03D-01..06` are COMPLETE. The next 03D candidate is `03D-07` optional-backend closure; it must not switch the default because 03D-06 did not meet the p50 Gate. `12E-09D-01..06` follow 03D; `12E-10A` and `13F-R1-01..05` remain separately prepared. 13B production acceptance remains blocked by unresolved device buildVolume/origin/axes and a 22-instance performance budget.
...
- TIFF writer state: handwritten remains the default uncompressed uint8 contiguous RGBWSV writer. An explicit `slicesoft-libtiff` lane links LibTIFF 4.7.1, deploys `tiff.dll` and its license, and routes stripped plus standard 16-aligned tiled output through LibTIFF. Legacy nonstandard tile dimensions retain the handwritten compatibility route. 03D-05 compatibility is PASS; 03D-06 is `GO_OPTIONAL` because the Release p50 Gate was not met. Do not switch the default writer without a future successful Gate and explicit authorization.
