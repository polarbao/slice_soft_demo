...
- Current task: Stage 16 engineering candidate is complete with S0 still the production default. 16D-02-R1 now permits explicit S3 `relief_heightfield` sampling for textured relief and single-material W/V relief; W/V keeps texture disabled, ordinary untextured RGB + S3 still fails closed, and Package/RIP strict is verified. P3 contact leveling and any S3 default switch remain unauthorized. Preserve SPI v1/11 exports/15 capabilities, Worker file contract, `p0.rgbwsv.2`, S2 path D and ViewData contracts.
