...
- Current phase: `03D-01` handwritten TIFF contract and Writer-only Release baseline is complete; `03D-02` LibTIFF dependency/CMake/Runtime integration is prepared but requires explicit authorization. `12E-09D` remains the next prepared material-control specialty after 03D.
- Latest completed milestone: `03D-01 handwritten TIFF writer contract and Release baseline`
...
- Current task: `03D-01` is COMPLETE. The only next 03D candidate is `03D-02`, which must wait for explicit authorization because it changes vcpkg/CMake/Runtime dependencies. `12E-09D-01..06` follow 03D; `12E-10A` and `13F-R1-01..05` remain separately prepared. 13B production acceptance remains blocked by unresolved device buildVolume/origin/axes and a 22-instance performance budget.
...
- TIFF writer state: `03D-01` added raw-tag contract tests and a Release Writer-only baseline for the handwritten, uncompressed, uint8 contiguous RGBWSV writer. Production CMake still does not find or link `TIFF::TIFF`, and no LibTIFF Runtime DLL is deployed. Do not begin `03D-02`, switch the default writer, or deploy new runtime DLLs without explicit authorization and the later stage gates.
