...
- Current phase: `03D-01..03` are complete; `03D-04` LibTIFF tiled/error-model work is READY. `12E-09D` remains the next prepared material-control specialty after 03D.
- Latest completed milestone: `03D-03 Writer interface and LibTIFF stripped Writer`
...
- Current task: `03D-01..03` are COMPLETE. The next 03D candidate is `03D-04`, covering LibTIFF tiled output, stable writer errors, sibling temporary files, and failure cleanup. `12E-09D-01..06` follow 03D; `12E-10A` and `13F-R1-01..05` remain separately prepared. 13B production acceptance remains blocked by unresolved device buildVolume/origin/axes and a 22-instance performance budget.
...
- TIFF writer state: handwritten remains the default uncompressed uint8 contiguous RGBWSV writer. An explicit `slicesoft-libtiff` lane links LibTIFF 4.7.1, deploys `tiff.dll` and its license, and routes stripped output through LibTIFF; tiled output still falls back to handwritten. Do not switch the default writer before 03D-04..06 gates and explicit 03D-07 authorization.
