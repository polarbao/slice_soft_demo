# slice_soft_demo Agent Docs

This directory uses progressive disclosure:

- `AGENTS.md` keeps repository-level always-on rules and routing short.
- `.agents/AGENTS.md` keeps project-level SliceSoft collaboration rules.
- `.agents/docs/**` stores longer project facts and policies.
- `.agents/skills/**/SKILL.md` stores task-specific workflows.
- `docs/slice` stores current formal PRD/DEV/DOC/ROADMAP documents.
- `docs/codex_task` stores current and archived Codex task cards/prompts.
- `docs/archive/2026-06-30_slicer_legacy` stores historical docs and completed-stage references.
...
- `SLICE_AI_SKILL_MASTER.md`: SliceSoft-specific master rules and strategy boundaries.
