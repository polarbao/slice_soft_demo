...
compression = none / packbits (default none; PackBits explicit experimental only)
...
12E: 12E-01..10D are complete in the approved scope. 10A binds production TIFF, 09A semantics, W/S/V and exact material closure by layerIndex/zMm. 10B passes 14 production rows and 3 expected strict blockers without fallback. 10C passes 36/36 Release measurements and RIP strict; Global is 1.826x-2.562x core, 2.244x-3.161x total, and 3.079x-4.304x peak memory versus Legacy. 10D closes the final report and user guide. Legacy remains default; Global remains explicit candidate. aishen/meigui/titian remain a 0/3 complex-relief coverage gap. `03D-01..07` remain the historical `GO_OPTIONAL` baseline; TIFF T-A-01..03 now supersede its Writer-default decision, with LibTIFF 4.7.1 as default and handwritten as an explicit legacy lane. `03E-01` is complete and `03E-02` is internally complete: `none|packbits` production configuration, manifest declaration, strict Reader, native preview, Qt and real-OBJ internal Gate pass, while external target RIP interoperability remains pending and default compression stays `none`.
...
15: ACTIVE on 2026-08-04. 15A-01..04, 15B-01..04, 15C-01, and 15D-01..04 are complete; automatic G1-G5, Quick CI zero drift and NFR-02 pass. Scope is Legacy full-volume RGB only: same-layer W carrier output for unprintable texture white, never S/V or ownership changes. The asynchronous texture preflight is conservative and non-blocking; 15C-02/03 remain. The strict RGB Profile and Global behavior remain unchanged. The candidate Profile stays disabled/diagnostic until G7.
16: ACTIVE / PARTIAL GO. 16A-01..06, 16B-01..03, 16C-01..03, 16C-05 and 16D-01..04 are implemented; 16C-05 still needs identical-request performance remeasurement. User authorized the 16C-06-MEMFLOW bounded streaming memory specialty on 2026-08-18: MF-00..03B3 are complete, covering RasterMemoryBudget, Legacy owned-layer transfer, compact occupancy range/caller-owned single-layer materialization, range-derived pre-shape support demand materialization, bounded outer-boundary/unsupported discovery, and non-production shape/footprint replay evidence. MF-03B4 base/varnish/material final replay remains only partially prepared. Retained dense remains the current production route until the relevant Gate passes. S3 remains explicit relief-heightfield only, S0 remains default, and P3/any default switch still require separate authorization.
```
