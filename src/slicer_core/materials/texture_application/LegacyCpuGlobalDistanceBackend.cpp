    ...
    const std::string& message)
{
    ...
    candidate.issues.push_back(MakeCpuIssue(code, message));
}
...
GlobalTextureFillPartitionCandidate LegacyCpuGlobalDistanceBackend::Evaluate(
    const GlobalTextureFillPartitionRequest& request) const
{
    const Clock::time_point totalStart = Clock::now();
    GlobalTextureFillPartitionCandidate candidate;
    candidate.available = true;
    candidate.backend = "legacy_cpu_global_distance";
    ...
    if (request.mesh == nullptr)
    {
        BlockCandidate(
            candidate,
            TextureFillPartitionErrorCode::CpuMeshMissing,
            "legacy CPU global distance candidate requires a transformed mesh");
        return candidate;
    }
    ...
    if (!voxelCount.has_value())
    {
        BlockCandidate(
            candidate,
            TextureFillPartitionErrorCode::CpuGridInvalid,
            "legacy CPU global distance candidate requires a finite positive grid");
        return candidate;
    }
    ...
    if (request.options.surfaceScope != "all_closed_surfaces")
    {
        BlockCandidate(
            ...
            "legacy CPU global distance candidate requires all_closed_surfaces");
        return candidate;
    }
        ...
        || std::abs(request.options.widthStepMm - 0.01) > 1.0e-12)
    {
        BlockCandidate(
            ...
            "legacy CPU global distance candidate requires a 0.01 mm width step");
        return candidate;
    }
        ...
        || !IsFinitePositive(request.options.baseMinimumWidthMm))
    {
        BlockCandidate(
            ...
            "legacy CPU global distance widths must be finite and positive");
        return candidate;
    }
    ...
    if (!topologyError.empty())
    {
        BlockCandidate(
            candidate,
            TextureFillPartitionErrorCode::CpuTopologyBlocked,
            topologyError);
        return candidate;
    }
    ...
    try
    {
        ...
        const Clock::time_point occupancyStart = Clock::now();
        PointInClosedMeshQuery occupancy(*request.mesh);
        PointInClosedMeshQueryStats occupancyStats = occupancy.GetBuildStats();
        PointInClosedMeshQueryOptions occupancyOptions;
        occupancyOptions.boundaryEpsilonMm =
            candidate.widthMetrics.epsilonMm;
        for (int z{0}; z < request.grid.depth; ++z)
        {
            for (int y{0}; y < request.grid.height; ++y)
            {
                ...
                {
                    const PointInClosedMeshResult occupancyResult =
                        occupancy.Classify(
                            GridCellCenter(request.grid, x, y, z),
                            occupancyOptions,
                            occupancyStats);
                    if (occupancyResult.ambiguous)
                    {
                        throw std::runtime_error(
                            "all deterministic occupancy rays were ambiguous");
                    }
                    if (occupancyResult.inside)
                    {
        ...
        candidate.performance.occupancyBuildMs =
            ElapsedMilliseconds(occupancyStart);
        candidate.queryStats.occupancyQueryCount =
            occupancyStats.queryCount;
        candidate.queryStats.occupancyVisitedNodes =
            occupancyStats.visitedNodes;
        candidate.queryStats.occupancyTestedTriangles =
            occupancyStats.testedTriangles;
        candidate.queryStats.occupancyFallbackRayCount =
            occupancyStats.fallbackRayCount;
        candidate.queryStats.occupancyAmbiguousRayCount =
            occupancyStats.ambiguousRayCount;
        candidate.queryStats.occupancyBoundaryPointCount =
            occupancyStats.boundaryPointCount;
        candidate.performance.occupancyQueryBytes =
            static_cast<std::uint64_t>(occupancyStats.estimatedBytes);
    }
    catch (const std::exception& error)
    {
        BlockCandidate(
            candidate,
            TextureFillPartitionErrorCode::CpuOccupancyFailed,
            std::string{"legacy CPU occupancy classification failed: "}
                + error.what());
        return candidate;
    }
    ...
    if (modelVoxelCount == 0U)
    {
        BlockCandidate(
            candidate,
            TextureFillPartitionErrorCode::CpuOccupancyFailed,
            "legacy CPU occupancy grid contains no model voxels");
        return candidate;
    }
    ...
    catch (const std::exception& error)
    {
        BlockCandidate(
            candidate,
            TextureFillPartitionErrorCode::CpuNearestSurfaceFailed,
            std::string{"legacy CPU nearest-surface query failed: "}
                + error.what());
        return candidate;
    }
    ...
    if (!std::isfinite(candidate.widthMetrics.maxInteriorDistanceMm))
    {
        BlockCandidate(
            candidate,
            TextureFillPartitionErrorCode::AllTextureThresholdUnavailable,
