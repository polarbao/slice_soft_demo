...
std::string StableTextureIdentity(const MaterialInfo& material)
{
    ...
    {
        return PathToUtf8(material.diffuse_texture_path.filename());
    }
    return PathToUtf8(material.diffuse_texture_path.lexically_normal());
}
...
std::string ComputeSceneResourceHash(const SceneModel& model)
{
    ...
    payload.append(PathToUtf8(model.model_path.lexically_normal()));
    payload.push_back('|');
