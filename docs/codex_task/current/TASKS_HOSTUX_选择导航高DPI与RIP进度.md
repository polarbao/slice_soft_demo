...
日期：2026-09-14。用户已授权覆盖正式 Release 目录、同步手册并按任务提交；未授权合并或推送，不关闭正在使用的软件。
...
### Current State
UX-00..13 COMPLETE（宿主定向 Gate）；选择轮廓、高 DPI、T 工艺切换、参数滚轮防误改及逐页滚动已修复，标签滚轮按用户新要求恢复。正式Release目录已覆盖，手册与配图已同步，相关修改按任务提交到当前功能分支；未合并或推送，不代表全仓/物理打印验收。下方较早轮次的“未覆盖/未提交”仅为当时历史记录。
### Target State
