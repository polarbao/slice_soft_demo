...
> 建立日期：2026-09-14；更新日期：2026-09-15；修订：v1.7；专项状态：PRODUCT INTEGRATION AUTHORIZED / G IN PROGRESS / PRINT ENTRY PENDING。
...
- 当前用户已授权实际合入 product；[G 产品合入准备](../../slice/DOC/DOC_PREP_LOGDUMP_G_产品合入与复用交付准备.md)完成，G02/G03 开始执行。前文 F 阶段“product 未移动”仅指当时状态，最终以 G 行为准；打印 E01B 前置缺失不变。
...
| LD-G02 | 日志专项合入本地产品分支 | G01 | IN_PROGRESS | - | 隔离树切换 product 并快进；不携带P0FIX修改、不推送、不删除分支 |
| LD-G03 | 产品构建、运行包与可复用SDK交付收口 | G02 | PREPARED | - | 清洁提交Release构建、相关回归与实际落盘、SDK清单核验；保留既有红灯和打印外部未验收边界 |
