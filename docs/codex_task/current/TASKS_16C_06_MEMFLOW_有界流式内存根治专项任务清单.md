...
| MF-13 | 包发布的读回全量校验 | **a+b 档 COMPLETE**：校验阶段`11,482 -> 5,272 ms`（**−54.1%**，安静机器、隔离口径）；已定性为 **CPU 界限**（CPU≈墙钟）| MF-12 | - |
...
**下一步：** §14.5 的候选 A（并行化逐层校验）**仍然可做且现在依据更强了**
（已确认 CPU 界限、单线程）。但先取本次的端到端数字，再决定是否值得引入并发。
