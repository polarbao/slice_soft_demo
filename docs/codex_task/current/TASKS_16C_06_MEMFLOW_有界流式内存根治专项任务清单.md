...
| MF-13 | 包发布的读回全量校验 | **NEW / 单项最大**：43.4 s / 90.1 s，见 §14.5 | MF-12 | - |
...
之上（源码注释明写）。**故本卡不是「关掉校验」，而是「换成不需要重新读盘的等价
校验」**，候选方向：

```text
A  写盘时单趟校验：写入器本来就持有即将写出的字节，边写边验，
   省掉整包重新读回。等价性最强（同一批字节、同一套判据），改动面在写入器。
B  只在发布后校验结构与标识（头、条带表、逐层字节数、哈希），不重新解码像素。
   代价低但**减弱了判据**，属产品决策，须用户裁定。
C  并发校验：校验与 compose 重叠。总 CPU 不变，只压墙钟；但当前 compose 已被
   屏障锁步，重叠空间需先测。
```

**前置：先确认 `ValidateSlicePackageArtifact` 到底验了什么**（是否解码像素、
是否逐层哈希），再判断 A 是否真的等价。**未测之前不动手** —— 本卡已四次因为
「先量后改」推翻自己的排序。
