...
> 版本：v2.5 ｜ 日期：2026-09-20 ｜ 用户授权：2026-08-17、2026-08-18、2026-08-25、2026-09-09、2026-09-20
...
| RIPFLOW-D | 矩阵、Runtime 迁移、本地收口和缺陷修复 | D-01..09 | COMPLETE |
...
C-04 -> D-01 -> D-02 -> D-03 -> D-04 -> D-05 -> D-06 -> D-07 -> D-08 -> D-09
D-03 + 外部材料 -> E-01/E-02
```
...
### RIPFLOW-D-09 20260917/18/20 供应方版本适配

**状态：COMPLETE / PASS（2026-09-20）**

**依赖：** D-08；用户补全 `RIPDLL_20260917`、`RIPDLL_20260918`、`RIPDLL_20260920` 并确认 0920 为最新版本。

**内容：** 盘点连续三个供应方版本并固定部署 0920。因三个新目录均缺 `CmykFiles`，来源合同升级为 v2，明确使用 0920 的 EXE/DLL/私有 TIFF 与 0909 的完整资源；provenance 分别记录二进制和资源来源。模块提升至 1.3.0，自检新增 `--verbose` 能力要求，生产命令默认不启用 verbose。0917/0918 仅作为中间版本证据，不形成 UI 选择或运行时回退。

**风险边界：** 三版公开头文件逐字节一致，可以保持既有进程外 CLI 接口；但 0920 的 `ripMode=1` 输出相对 0917/0918 发生变化，不能只把它视为日志更新。该变化已按最新 DLL 重新跑宿主合同与矩阵，不代表物理工艺已获外部验收。禁止在缺少资源的 0920 目录内手工混入文件后丢失来源记录。

**验收：** source v1/v2 与非法 pin 门禁；11 文件模块打包、自检和双来源 provenance；中文路径默认/verbose 行为；两档墨量 x 五档颜色宿主矩阵；结果合同、源输入不变和诊断隔离；RIP 定向回归。

**实际结果：** Debug 全目标串行重建 PASS；RIP 定向 CTest 11/11 PASS，合同正例 8 / 负例 13 PASS。中文路径默认与 verbose 均输出 3/3 层，verbose 仅额外开放 3 条调试日志；宿主 10/10 组合 PASS。Runtime 模块 11/11 文件、自检和 provenance PASS，记录 `RIPDLL_20260920` 二进制与 `RIPDLL_20260909/CmykFiles` 资源。0917/0918 两档逐层一致，0920 mode0 一致、mode1 三层均变化。按用户授权，Runtime GUI 自测和非 RIP 核心档既有问题不纳入本卡修复；外部分发与打印侧验收继续 `BLOCKED_EXTERNAL`。详见 `docs/slice/REPORT/REPORT_RIPFLOW_D09_20260917至20260920版本适配验证.md`。
...
| 2026-09-20 | v2.5 | D-09 完成：补齐 0917/0918/0920 差异链，固定 0920 二进制与 0909 资源的双来源模块 1.3.0；verbose、中文路径、宿主 10 档矩阵、11/11 定向 CTest 和 Runtime 模块验证通过；外部状态不升级 |
