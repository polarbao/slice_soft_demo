...
> 文档状态：**ACTIVE — H-D 待点名 / H-E 已授权分三批**
> 版本：v2.6 ｜ 日期：2026-08-08
...
```text
H-A   ABI 场景生命周期（4 卡）—— 已完成
H-B   宿主业务流程 UI（8 卡）—— 已完成
H-C   移植交付物（3 卡）—— 已完成
H-D   视图接线与端到端可操作性（6 卡）—— 【当前可开工组】不改契约，无需新授权
      H-D-01 俯视画布 · H-D-02 3D+相机 · H-D-03 三车道拖拽
      H-D-04 刷新事件 · H-D-05 打开包目录 · H-D-06 人工验收
H-E   参数深度与导入补齐（6 卡）—— ✅ HQ-09=乙 已授权，分三批
      E1  H-E-01 STL 导入 · H-E-03 支撑参数编辑
      E2  H-E-04 材料工艺 Profile · H-E-05 生产纹理设置
      E3  H-E-02 批量导入 · H-E-06 白区预检接入
```

> ✅ **2026-08-08 用户裁决**
>
> **HQ-09 = 乙**：参考宿主的目标水位是「**等价于切片库封装前的切片软件**」，**分三批执行**。
> **HQ-10 = 甲**：场景/项目保存与加载**归 PrintApp**，参考宿主不实现。
> 权威记录：`DOC_DECISION_HOSTFLOW_H_E_R1_参考宿主目标水位裁决.md`
>
> ⚠️ **批次门**：E1 完成后必须复核实际可用性，再决定 E2 是否照原样执行。
> **不得三批一次性排完** —— H-E-03 会确立「宿主 Profile 可编辑段」的结构形态，
> E2 在其上扩展；E1 后若需调整框架，E2 尚未开工则返工成本为零。
>
> ⚠️ **HQ-10 的直接后果**：参考宿主**重启后场景丢失、须重新导入**是
> **已裁决的预期行为**，不是缺陷，**不得作为 bug 提报，也不得顺手实现工程保存**。

### ⚠️ H-D 组必读：为什么会有这一组

十五张卡的**业务与数据链路**都通了，但**视图显示链路从来没有接上**：

```text
ViewWorkspaceWidget.cpp:13-27     两个画布是 QLabel + 静态文字，零像素输出
HostMainWindow.cpp                对 m_workspace 只有 SetMode / ShowViewError /
                                  SetSelectedInstances 三处调用
SceneRenderPolicy 等 7 个类        在 app 内的唯一引用者是 CapabilityCoverageWorkflow
                                  （批处理自检），不是可见 UI
```

`REPORT_14E_04d` §5 写「渲染策略接线由打印侧集成时复用」——**该延期作废**。
参考实现必须自己把这段做完，否则最难的一段留给了移植方。

**接线时不要重写策略类。** 它们已通过 UI-M1..M13 门禁；
H-D 是**把已有输出接到画布并处理刷新时机**，不是重新实现渲染。
...
```text
① 「离屏测试通过」≠「界面上看得见」
   14E-04c/04d 的门禁全是离屏渲染，全绿但画布是空的。
   H-D 每张卡的验收【必须把 app 真的跑起来看】，并归档截图。

② 打开包目录只能用作业返回的 packageDir
   不得自行拼接输出目录 + 时间戳去猜。路径缺失就禁用按钮并说明原因。

③ 接线不得破坏 UI-M1 / UI-M7 的零跨 DLL 调用
   拖拽与相机期一次 DLL 调用都不能有。接线后【重新实测计数】，不得沿用旧数据。
```
...
✗ H-E 跨批次开工（未过批次门就动 E2/E3）           → 违反分批裁决
✗ 为 H-E 新增 ABI 面或改读内部 slicer_scenarios    → 推翻 HQ-08-A
✗ 实现场景/项目保存加载                            → HQ-10 已裁决归 PrintApp
✗ 把 apps/slicer_ui_host_sim/ 加入行数白名单        → 绕过门禁，应拆文件
✗ 发现某项主干能力既不在 H-B/H-D/H-E，也不在 C 桶  → 分类有遗漏，回来复核
...
【全部业务流程跑通后】
第 4 批   H-C-01 → H-C-02 → H-C-03（已完成）

【当前待办】
第 5 批   R-A-01 → H-D-01 → H-D-05
          ⚠️ 顺序执行，不要真并行：H-D-01 与 H-D-05 实现文件不重叠，
             但都要改 HostMainWindow.h；且并行会破坏「每卡独立门禁 + 单独提交」证据链。
             R-A-01 零源码改动，先跑它 —— 结果决定 H-D-01 要不要带 LOD 兜底。
第 6 批   H-D-02 → H-D-04
第 7 批   H-D-03 → H-D-06
第 8 批   H-E-01 → H-E-03      ── E1 批次门 ──
第 9 批   H-E-04 → H-E-05      ── E2 批次门 ──
第 10 批  H-E-02 → H-E-06      ── E3 完成后回填 H-C-03 的 known_trim ──
```

> 🔴 **接线前必看：`HostMainWindow.cpp` 当前 484 行，门禁上限 500 行，只剩 16 行。**
> 按既有拆分惯例（`HostMainWindowScene/Job/Result/Profile/Settings/State.cpp`）：
> H-D-01 新建 **`HostMainWindowView.cpp`**；H-D-05 的槽放入既有 **`HostMainWindowResult.cpp`**。
...
| 日期 | 版本 | 变更 |
|---|---|---|
| 2026-08-08 | v2.6 | 用户裁决 **HQ-09=乙**（等价水位，分 E1/E2/E3 三批）与 **HQ-10=甲**（场景保存归 PrintApp）。H-E 解除开工封锁并按批次重排，写入批次门与「重启后场景丢失是预期行为」的约束；新增四条停止条件（跨批次、新增 ABI 面、实现工程保存、加行数白名单）；开工顺序补第 8–10 批；补 `HostMainWindow.cpp` 484/500 行的拆分指引与 H-D 顺序执行说明。 |
| 2026-08-08 | v2.5 | 复核发现视图显示链路未接线（画布仍为占位 `QLabel`，渲染/交互内核只被批处理自检引用），结果页无「打开包目录」。作废 `REPORT_14E_04d` §5 的接线延期决定，新增 **H-D 组 6 卡**并写入坑 7（离屏测试通过≠界面可见 / `packageDir` 不得猜测 / 接线后须重测零跨 DLL 调用）与第 5–7 批开工顺序。文档状态回退为 ACTIVE。 |
...
| 2026-08-08 | v2.0 | H-B-07 完成生产包校验、摘要、逐层生产 TIFF 预览、命名报告和 RGBWSV 通道图；场景生产 Writer 补齐真实 `perInstance/profileEcho`，不放宽 Facade。Debug/Release 联合门禁、宿主边界与源码尺寸守卫通过。下一卡为 H-B-08；H-C-01/03 已解除 H-B-07 前置。 |
