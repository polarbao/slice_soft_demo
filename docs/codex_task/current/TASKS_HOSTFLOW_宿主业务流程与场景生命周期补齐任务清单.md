...
> 文档状态：**ACTIVE — H-D/H-E 与 H-F-01..13 完成**
> 版本：v5.7 ｜ 日期：2026-08-18 ｜ 激活日期：2026-08-07
...
| **H-C-03** | 并排 A/B 对照报告：主干 UI 与宿主模拟对同一模型同一 Profile 语义的行为差异清单 | H-B-07 | 14 条差异覆盖 9 个维度；Debug/Release 对照门禁通过；H-D-06 回填真实显示维度 | **COMPLETE（2026-08-10）** |
...
| **H-D-04** | **视图刷新事件接线**：导入 / add / remove / transform / applyGridLayout / `sceneContext` 变更后自动 Refresh；按 `DOC_SCHEMA_14` §6 失效规则，**`worldMatrix` 变化不得使网格缓冲失效** | H-D-01；完整验收需 H-D-02 | 22 实例排版后视图一次刷新到位；`MeshUploadCount` 在纯变换后**增量为 0**（实测）| **COMPLETE（2026-08-10）** |
...
| **H-D-06** | **端到端人工可操作性验收**：按 §4 口径逐步走通七步流程并归档截图；把「显示」维度回填 `REPORT_HOSTFLOW_H_C_03` | H-D-01..05 | Release 人工完成导入、显示、选择、变换、排版、切片与生产包结果七步；Debug/Release 七组 UI self-test 全部通过；A/B 对照新增「显示」维度 | **COMPLETE（2026-08-10）** |
...
| **H-E-05** | E2 | **生产纹理设置**：对照 `ProductionTextureSettingsPanel` + `Contract` + `Model` | H-E-04 | 纹理应用模式、UV 策略等进入有效 Profile；与 Stage 15 `unprintable_white_*` 字段协同 | **COMPLETE（2026-08-10）/ E2 GATE PASS** |
| **H-E-02** | E3 | **批量导入**：多选文件一次导入并逐个 `addInstance`；对照主干 `SceneBatchImportController` | H-E-01 | 单次原子 Commit 或明确的逐项失败语义；**部分失败不得留半个场景** | **COMPLETE（2026-08-10）** |
| **H-E-06** | E3 | **纹理白区预检接入**：对照主干 `TextureWhitePreflightService`（Stage 15 15C 成果）| H-E-05 | 导入/切片前给出保守告警；结果携带 scene/revision/contentHash，切换场景后旧结果丢弃（Stage 15 坑 7）| **COMPLETE（2026-08-10）/ E3 GATE PASS** |
...
| **H-F-02** | 修复 3D 左键纵向环绕跨越近极点阈值时的相机基向量跳变 | H-D-02 完成；用户报告 3D 拖拽显示跳变 | 视图矩阵和 Pan 共用连续的世界 Z 向上基向量；跨越旧 `abs(forward.z)=0.98` 阈值时右轴点积大于 `0.99`；Debug/Release 三维合同与宿主画布门禁通过 | **COMPLETE（2026-08-10）** |
...
| **H-F-08** | 定位并优化五模型新版切片报告统计与严格包校验耗时 | H-F-05/H-F-07 完成；用户提供 2 分 20 秒五模型生产包 | 区分层厚变化与校验开销：当前包 0.021 mm/263 层，不得与 0.038 mm/145 层直接比较；保留报告和发布前严格校验，优化 RGBWSV 分层统计、TIFF 整文件读取与 stripped TIFF 统计热点；现有 263 层 PackBits 包严格读取保持 PASS，三次均值由修改前约 19.60 s 降至 9.70 s；统计语义、校验和、生产 TIFF、SPI、Profile 与包协议不变；Release 定向 CTest 通过并部署运行时 | **COMPLETE（2026-08-14）** |
| **H-F-09** | 修复实时计时从十几秒出现并跳变，区分宿主连续墙钟与 Worker 快照 | H-F-05 完成；用户要求从 0 秒连续计时 | `QElapsedTimer` 从作业提交成功起作为 UI 连续墙钟，每次 100 ms 轮询均刷新；Worker `elapsedMs` 保留为独立快照，不再冒充连续计时。阶段估算使用宿主时钟边界，Worker 未提供快照时明确显示“未提供”；第二次作业重新从零计时。Release 控制器、设置面板和两项 UI smoke 定向门禁 4/4 PASS | **COMPLETE（2026-08-17）** |
| **H-F-10** | 定位并优化 Reality 十模型批量导入阶段的场景碰撞准入耗时 | H-B-01/H-A-04 完成；用户提供真实模型目录并报告导入变慢 | 确认 10 个 OBJ 共 160,944 三角面，解析、哈希、预检和双视图刷新均非主因；初始同位 addInstance 提交触发 45 对实例的逐三角形笛卡尔积精确裁剪。以确定性三角形 BVH/AABB 宽相位收窄候选，保留原精确多边形裁剪权威和碰撞语义；同目录 Release 导入由 267.265 s 降至 2.070-4.615 s，含排版与双视图总时长由 268.276 s 降至 2.898-8.004 s；定向 CTest 4 项 PASS；Runtime 安全就地部署完成，构建/运行文件哈希一致且自检 PASS | **COMPLETE（2026-08-17）** |
| **H-F-11** | 恢复 STL 实例的可见触底操作，并将三轴旋转后自动触底固化到场景变换合同 | H-E-01/H-B-03 完成；用户报告 `123.stl` 导入后无法自动铺底，变换面板无触底入口 | 保留源模型右角候选自动定向；新增可持久的 `landOnBuildPlate` 实例变换和同名操作，由模块根据变换后包围盒计算 Z 补偿，保证 `minZ=0`；宿主导入默认开启，变换面板提供“变换后自动触底”和“将选中实例触底”，同时移除核心不支持的自由 Z 增量；`123.stl` 真实资产、合同、核心变换和 Qt UI 定向门禁通过，Runtime 已部署 | **COMPLETE（2026-08-17）** |
| **H-F-12** | 修复 `123.stl` 孤立微小角标误导触底，并开放受控 Z 轴平移 | H-F-11 完成；用户复测发现可见主体仍悬空，并要求确认/补齐 Z 轴移动 | 识别原文件微小孤立角标与主体落点差异；触底基准改为显著连通组件最低点但不删除进入 SceneModel 的几何；DTO v1.13 以加法方式开放 Z 平移，非零 Z 自动关闭持久触底，显式触底清零 Z 偏移。当前 STL 导入器会先过滤 12 个退化三角面，`123.stl` Runtime 实测保留 41,258 面、`worldZ=0`、权威 `bboxZ=0..4.48658 mm`；孤立组件语义由保留标记的合成回归覆盖。合同/核心/Facade/复用/Qt/真实资产定向门禁通过；SPI、RGBWSV、TIFF 和 Profile 不变 | **COMPLETE（2026-08-18）** |
...
| 2026-08-10 | v3.9 | 完成 H-D-06：Release 参考宿主人工走通导入、双视图显示、选择、变换、排版、切片和生产包结果七步并归档 7 张截图；Debug/Release 七组宿主 UI self-test 全部通过。修复工作区状态 smoke 仍按 schema v1 判断、与当前 v4 不一致的问题。H-C-03 机器矩阵新增“显示”维度，更新为 14 条 / 9 维。打印侧 ACK、目标 RIP 和物理打印仍延期。 |
...
| 2026-08-18 | v5.6 | 完成 H-F-12：H-F-11 的完整包围盒 `minZ=0` 不足以表达带孤立微小组件的主体落点。触底改以显著连通组件最低点为基准，并只在 SceneModel 变换阶段工作；开放 Z 增量，非零 Z 关闭自动触底，显式触底清零手动 Z。DTO 升至 v1.13，旧零 Z 变换哈希与旧 scene 文档兼容。Release 定向门禁及 Runtime `123.stl` 实测 PASS：既有 STL 导入清理后保留 41,258 个非退化三角面，`worldZ=0`、权威 `bboxZ=0..4.48658 mm`；保留孤立标记的合成回归验证主体触底语义。Runtime self-test PASS。部署时因运行目录被占用采用脚本内置的就地不可变载荷更新并保留 `output`。 |
| 2026-08-17 | v5.5 | 完成 H-F-11：根因是既有 `autoOrient` 只处理源模型的 X/Y 右角候选和导入时触地，实例变换合同只允许 XY 平移、Z 旋转和缩放/镜像，宿主反而显示了无法提交的 Z 增量。受控扩展实例 X/Y 旋转与持久化 `landOnBuildPlate`，模块统一以变换后包围盒计算 Z 补偿；导入默认触底，且提供显式复位按钮。Release 合同/核心/宿主定向门禁 10/10 PASS；`C:\Users\admin\Downloads\123.stl` 真实资产门禁 PASS，权威包围盒 `minZ=0`；Runtime 安全就地部署，宿主 `--self-test` PASS，构建/运行三件核心产物 SHA-256 一致。H-F-10 的 Runtime 占用阻塞也随本次部署关闭。全局源码尺寸脚本因同工作树中与本卡无关的 RIPFLOW/版本治理未完成文件及 `slicer_cli/main.cpp` 增长而报错，H-F-11 修改文件未触发新的尺寸违规。 |
| 2026-08-17 | v5.4 | 实现 H-F-10：Reality 十模型共 160,944 三角面，资源解析、哈希、快速预检仅约 2 s，实际停顿来自模型首次以同一单位变换提交时，场景准入对 45 个实例候选对执行逐三角形笛卡尔积精确裁剪。引入确定性三角形 BVH/AABB 宽相位，仅收窄候选，原精确多边形裁剪仍为最终权威；同目录 Release 五次加速后导入为 2.070-4.615 s，中位数 2.277 s，修改前为 267.265 s。碰撞、场景生命周期、导入和三维画布定向门禁 4 项 PASS。最终 Runtime 部署被正在运行的 `slicer_ui_host_sim.exe` 占用阻止，状态保留 `RUNTIME_DEPLOY_PENDING`。另发现既有 `scene_facade_14b03_unit_tests` 仍断言 0 mm 边距，与 H-F-03 已冻结的 10 mm 默认边距不一致，本卡未改该历史测试。 |
| 2026-08-17 | v5.3 | 完成 H-F-09：定位跳秒根因为 UI 直接显示 Worker 仅在新进度快照发布时更新的 `elapsedMs`。改用宿主 `QElapsedTimer` 从提交成功起连续刷新，Worker 时间独立显示；阶段边界估算改用宿主墙钟，Worker 未提供时不再用宿主时间伪装。第二次作业重新从零计时。Release 定向 4/4 PASS，Runtime 发布后构建/运行文件 SHA-256 一致且 self-test PASS。 |
| 2026-08-14 | v5.2 | 完成 H-F-08：五模型 2 分 20 秒样本实际使用 0.021 mm/263 层，而较早同布局样本为 0.038 mm/145 层，层数增加 81.4%，因此并非同参数性能回归。保留生产报告与发布前严格包校验，消除层统计逐字节取模/边界访问，并把 stripped TIFF 校验改为整文件预分配读取、连续拷贝和固定六通道批量统计。现有 2499×623、263 层 PackBits 生产包严格读取三次均值由修改前约 19.60 s 降至 9.70 s（约 -50.5%），报告统计热点独立微基准为 2.33×；精确通道统计/校验和回归与 Release 定向 CTest 4/4 PASS。运行时已部署；Visual Studio/NMake 发布构建分别暴露既有 MSBuild 节点停滞与 NMake 中文手册路径复制失败，最终采用已通过定向构建测试的 Release Ninja 产物部署并完成 self-test。 |
...
| 2026-08-10 | v4.6 | 完成 H-F-02：删除近极点处在世界 Z/Y 向上之间硬切换的相机基向量逻辑，视图矩阵、平移和光标缩放统一使用连续世界 Z 向上基；新增跨旧阈值纵向环绕连续性断言。Debug/Release 的 `hostflow_hd02_three_d_canvas` 与 `slicer_stage14e04c_three_d_contract_test` 各 2/2 PASS。Release 宿主可执行文件因正在运行而未重新链接，测试目标已完成 Release 编译。 |
