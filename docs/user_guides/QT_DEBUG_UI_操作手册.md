...
| 页面 | 作用 |
|---|---|
| 层预览 | 生产层检查入口，按真实 `layerIndex` 查看 RGB/W/S/V/occupancy/diagnostic 等 per-layer 视图，可用像素探针判断 RGBWSV 六通道值。 |
| 报告 | 查看 manifest 和 reports JSON 摘要。 |
| 曲线 | 查看通道/层统计曲线。 |
| 配置 | 编辑当前配置 JSON 和常用参数。 |
| 叠加预览 | 材料关系合成视图，只把同一层的 RGB 与支撑/白墨/光油伪彩叠加，适合检查相对位置。 |
| 原始预览 | 原始 preview 文件浏览器，直接查看输出包 `preview` 目录中的 PNG/PPM 调试图。 |

### 5.1 三种预览是否可以整合

短期不建议把三者直接合成一个无差别页面，因为它们的数据来源不同：
...
后续可把它们整合为一个“预览”工作区，并用模式切换：

```text
生产层检查 / 材料叠加 / 原始文件
```

但在当前调试 UI 中保持拆分更利于定位问题。
