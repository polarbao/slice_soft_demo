...
4. 在“模型”页确定两个导入开关和批次原点，随即在同一页批量导入 OBJ、3MF 或 STL。
   （2026-09-20 起这两个开关与“添加模型”同页。此前它们在“变换与排版”，
   而开关是在**导入执行的那一刻**被读的——按标签从左到右操作的用户会先导入、
   后才看到开关，勾选已经不起作用。）
...
6. 必要时在“变换”页调整实例，回到“模型”页重新执行规则排版。
7. 在“切片作业”确认已就绪后开始切片。
...
| 单材料缩裹｜整模 T 实体｜下表面支撑 | **整个模型都是缩裹材料**，只写 T；无需 mtl，也不做颜色匹配。见 §4.2.2 |
...
### 4.2.2 单材料缩裹｜整模 T 实体（2026-09-20 起支持一版多件）

选中这条预设即表示**整个模型都是缩裹材料**，与「单材料光油 V」「单材料白墨 W」
是同一类工艺，区别只在内容落到哪个通道：光油落 V、白墨落 W、缩裹落 T。

它与 §4.2 表格里那批「｜缩裹 T 通道」变体**不是一回事**：

| | §4.2 的缩裹变体 | 本预设 |
|---|---|---|
| 缩裹区域怎么认 | 按材质漫反射色 `Kd` 精确匹配 | **不认材质，整模即缩裹** |
| 对资产的要求 | 必须有 mtl 且颜色与工艺一致 | **无 mtl 的纯几何 obj 也可用** |
| 甲片部分 | 仍走 RGB / W / V | 没有「甲片部分」，整模只写 T |
| 拓扑要求 | 缩裹区域须闭合、自交不超限 | **不要求流形或无自交** |

最后一条是实务上的关键差别：整模模式不经体积求解，直接取模型自身的占位，
因此一批在按颜色匹配下因拓扑被拒的资产，换这条预设可以正常切片。

#### 一版多件

**2026-09-20 起，这条预设支持在一个版面上排多件模型，一次切成一个包**——
此前它要求版面上恰好一个可见模型，多于一个会以
`PM-SLICER-LAYOUT-0022 · slice.rgbwsvt requires exactly one visible scene instance`
直接失败。

现在的行为与单材料光油 / 白墨一致：画幅取整版最大外包围区域，所有实例合成到同一张
画布上，产出**一个** `p0.rgbwsvt.1` 包。

![整版缩裹的单层 T 平面](assets/packaged_slicer/28_整版缩裹单层T平面.png)

上图是一版 12 件的某一层 T 平面（全画幅 1004×126，黑色为打印）。同一层的
R/G/B/W/S/V 六个通道全空——缩裹像素与前六通道互斥，这是 T 通道的既定语义。

把同一版、同一层分别用「单材料光油」和「单材料缩裹」各切一次，两者的打印像素
**逐像素完全相同**，差别只在落到 V 还是 T：

![光油 V 与缩裹 T 同形对照](assets/packaged_slicer/29_光油V与缩裹T同形对照.png)

上半为单材料光油的 V 平面，下半为整模缩裹的 T 平面，各 8826 个打印像素。

#### 两条约束

- **同层 XY 投影不得相交**。两件模型在同一层的截面若有重叠像素，整单拒绝
  （`InstanceOverlap`）。这不是缩裹独有的限制，六通道整版一直如此；正常排版
  留有间隙即可。
- **至少要有一个可见模型**。零可见实例仍然拒绝。

#### 耗时

同一版面下，整模缩裹比单材料光油慢约两成。这是结构性的：七通道比六通道多约 17%
的 TIFF 字节，另加整版七通道装配那一遍全画幅扫描。不是缺陷，也无需调参。
...
“RIP 设置”分为“参数 / 路径 / 手动 RIP”。
（“变换与排版”原本也分两页，2026-09-20 把“导入与排版”迁到“模型”页后，
该标签只剩实例变换，已更名为“变换”且不再分页。）
...
图 26：导入开关、批次原点与规则排版。
⚠ **该截图 2026-09-20 起已失真**：这一组已迁到「模型」页并紧邻「添加模型」，不再是「变换与排版」的子分组。待重拍。
...
多图层工艺的命名推导、透明转光油及薄面阈值会随工作区保存，重启后恢复。
旧版保存时遗漏这些字段的标准“多图层透明”预设会按其原有规则补齐；自定义材质配置不会自动改成多图层工艺。
“开始切片”置灰时，查看“切片作业”中的具体原因或将鼠标停在按钮上查看提示；不要仅反复切换工艺。
...
**当前接入支持六通道 RGBWSV 与含缩裹 T 的七通道 RGBWSVT，二者均须为 unsigned 8bit、contiguous、stripped TIFF。**
完整切片包会按 `p0.rgbwsv.2 + R,G,B,W,S,V` 或
`p0.rgbwsvt.1 + R,G,B,W,S,V,T` 成对校验；手动 RIP 以首层固定通道数，后续层不得混用六、七通道。
