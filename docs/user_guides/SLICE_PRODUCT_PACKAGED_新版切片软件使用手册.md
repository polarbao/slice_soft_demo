...
> 日期：2026-09-07（同步非打印定位材质规则，见 4.2.1；保留多图层透明→光油及自动优先级说明）
>
...
“切片设置 -> 常用工艺预设”提供七类六通道组合，另有四个“｜缩裹 T 通道”变体：
...
> **待补配图**：`assets/packaged_slicer/12_多图层透明光油工艺预设.png`
> 取景：工艺预设下拉展开，可见“多图层透明→光油”一项。
> 生成方式：`scripts/CaptureHostUiScreenshot.ps1`（按进程名定位宿主窗口并裁剪）。
...
“切片设置 → 输出画幅 → 补齐至 X=0”默认关闭。
当模型全部位于平台 X=0 右侧，勾选后将原点到切片左边界的空白一起写入每层 TIFF；不移动模型，
也不补 Y 或改变 Z 触底。补白区域不会喷墨，结果预览与 TIFF 使用同一扩大后的画幅。
按整像素保留原采样位置，左边界可能在 0 左侧不足一个像素，不会靠移动模型伪造精确 0。
已覆盖 X=0 时不重复补白；不放宽越界检查。支持 RGBWSV 六通道及 RGBWSVT（含 T）场景工艺，
T 工艺仍遵循原有单可见实例准入，不增加 T 材料、不改变 T 与其他通道的独占规则。
旧输出包不自动修改，需要重新切片；补白会增加输出文件大小。
...
所以 `L1` 整层压过 `L2` 整层；只有同层时才由“透明优先”决定。命名违规或算出撞号都会
在切片开始时以 `E_MATOPQ_LAYER_NAME_INVALID` / `E_MATOPQ_LAYER_PRIORITY_COLLISION`
失败，不会静默继续。

> **待补配图**：`assets/packaged_slicer/13_多材质纵深自动优先级.png`
> 取景：展开“多材质纵深”段，勾选“启用”与“按材质命名自动推导优先级”，
> 可见两个材质名槽为停用态。生成方式同 4.2.1。
...
### 11.3.3 内嵌部件在切片数据里消失（无报错）

切片正常结束、退出码正常、报告 `errors` 为空，但内嵌的部件在切片数据里看不到。

这**几乎总是材质命名问题**：被包裹的素材与包裹它的介质用了同一层号。同层时由类别次序
裁决，透明素材胜出并占据全部重叠区，被包裹的素材因此一个像素都拿不到。

处理：把包裹介质的层号改大（`trans-L1` → `trans-L2`），被包裹素材保持 `L1`。详见 4.2.1。

判定方法：查看生产包 `reports/material_volume_report.json`，把各层的
`ownerPixelsByMaterial` 按材质累加。**某个已声明的材质累加结果为 0，即表示它被完全压掉。**
...
| 耗时集中处 | 含义 | 处理 |
|---|---|---|
| 逐列求交 / 网格准备 | 多材质纵深的材质所有权求解 | 与几何面数、栅格列数成正比。参考量级：10 万面模型约 50 s |
| 图层合成 | 逐层通道写入 | 与层数成正比，降低层数或层厚可缓解 |
| TIFF / 预览写入 | 编码写盘 | 检查输出目录是否在慢速磁盘或网络位置 |

多材质纵深工艺（4.2.1）比普通工艺慢是**预期的**：它要按 Z 解析每个 XY 列上的材质归属，
而普通工艺逐列只取一个顶面材质。参考量级（635×600 DPI，单机串行）：

```text
 2 万面 / 474 层    约 24 s
 3.7 万面 / 124 层  约 22 s
10.4 万面 / 129 层  约 50 s
```

若明显超出该量级、且耗时集中在逐列求交，属于已知的可优化项，处理依据与触发条件见
`docs/codex_task/current/TASKS_MATVOL_PERF_逐列求交性能专项任务清单.md` §6。
**报告问题时请附上完整耗时字段**，否则无法判断瓶颈位置。
