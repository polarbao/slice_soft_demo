...
3. **失败集合与基线一致**。比**集合**不比**数量**（`AGENTS.md` 8c）——数量相同可能是
   ...
   **已知抖动项另算一档**（2026-09-18 第一次真正执行本闸门后收窄）：
   ...
   | 项 | 抖动原因 | 处置 |
   |---|---|---|
   | `module_logging_integration_tests` | `-j8` 下耗时膨胀约 3.5 倍（隔离 4.3~7.4 秒），撞上 `SliceSoftDiagnostics.cmake` 给它设的 `TIMEOUT 60`。2026-09-18、09-20 各红过一次，隔离与复跑均全绿；09-20 第三次隔离重跑 3/3 通过、3.39~3.54 秒；09-22 合并闸门第四次，隔离 3/3 通过、3.62~3.79 秒，距阈值 16 倍余量，且已实测全量档之后源码一字未改——同一份二进制在那次全量档里跑绿过本测试 | 隔离重跑确认后放行 |
