...
**`product/legacy-slicer` 保留原名，不参与本次迁移。**

> ⚠ **本节初稿的理由是错的，2026-09-18 实测后更正。** 初稿写「重命名被其它工作树检出的分支
> 会破坏那个工作树」——**不成立**。实测：改名退出码 0，那个工作树自动跟随新名，
> 未提交改动（已跟踪的修改与未跟踪文件）全部完好。
>
> 真正被工作树挡住的是**删除**，不是改名：
>
> ```text
> $ git branch -d feat/x
> error: cannot delete branch 'feat/x' used by worktree at '...'
> ```
>
> 规范 2.6 那条「工作树占用的分支删不掉」说的是删除，它没错——是本文初稿把两者混为一谈了。

去掉那条错误理由后，保留原名的依据变成下面三条，**都与工作树无关**：

**① 它没有下游，`develop/*` 这个名字会说谎。**
`develop/<名字>` 指的是**集成线**：功能在上面累积，到里程碑合入发布线。
`develop/packaged-slicer` 的下游是 `main`。而 legacy **没有下游**——
它 2026-08-05 在 `b5fc0fb3` 与主线分叉，此后主线走了 **495 条**、它走了 **2 条**，
是另一个产品（手工运行时 vs 封装能力包），**不会也不该合进 `main`**。
它本身就是 legacy 这条线的发布线。叫它 `develop/*` 等于宣称一个不存在的拓扑。

**② 三档闸门在那条分支上根本不存在。**

| 分支 | `slicesoft-debug-core` / `fast` / `full` |
|---|---|
| `main` | 三个都有 |
| `product/legacy-slicer` | **一个都没有**（`CMakePresets.json` 里查无此项） |

集成线的正当性来自「三档闸门耗时差一个量级」（见规范第一节）。
预设都不存在，这个论证在 legacy 上无从谈起。

**③ 没有东西可累积。**
自 2026-08-05 分叉以来该分支只有 **2 条提交**，都在 2026-08-17，此后一个月无更新。
规范给集成线的收益是「几个功能累积起来、每次只付 3 分钟而不是 16 分钟」——
2 条提交 / 6 周，没有可累积的量。在这里加一条集成线，
正是规范自己警告过的那种**空转**（「初版把 develop 当成开发主分支，那在本仓会空转」）。

**如果将来要对称**，正确做法不是改名，而是等 legacy 具备两个前提再说：
把三档预设移植过去，且它的提交密度高到累积有收益。届时形态应是
「`product/legacy-slicer` 作发布线 + 新建 `develop/legacy-slicer` 作集成线」，
而不是把现有这条改名——改名会把**发布线**错标成集成线。

迁移后 `product/` 前缀下只剩它一条。这个不对称**反映的是真实拓扑**：
`main` 是仓库默认分支兼封装线的发布线，legacy 是一条独立的旁线。
