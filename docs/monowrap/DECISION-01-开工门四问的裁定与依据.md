...
> 本文回答开工门 G-1..G-4，**每条结论都附实测或代码位置**。

---

## 裁定结果（用户 2026-09-20）

用户原话：「问题方案可按照你的建议进行开发处理」。
即下列四条建议**全部采纳**，开发按本文执行：

| 门 | 裁定 | 落到哪些任务 |
|---|---|---|
| **G-1** | **方案 B**：T 通道内部合成材质名，不动 `MaterialVolumePlan` 与 MATVOL 共用的 fail-closed | MONOWRAP-04、05 |
| **G-2** | **方案一**：同版本放宽（`const`→`enum` + 条件 `minItems`），不升 `.2` | MONOWRAP-07 |
| **G-3** | **整盘一次切**（场景模式）；场景入口是否接通 T 通道须先实测 | MONOWRAP-01b、08 |
| **G-4** | 不成立，协议锁死为 0，无需裁定 | — |

**G-2 的放行前提未变**：必须先实测既有 738 产物基线逐字节不变（MONOWRAP-03）。
**该前提不过，G-2 要重新评估**，不能因为已裁定就跳过。
