...
12E-08A 已关闭 classification-to-raster 缺口，12E-08B 已关闭完整材料 diagnostic closure
缺口。当前还有两项生产证据缺口：

```text
1. 默认 OpenVDB OFF 的 CPU candidate 尚无真实模型 Release 性能和内存准入阈值；
2. 新 Profile 的 production package、RIP strict、旧 Profile TIFF 不变性尚无实际证据。
```
...
12E-08B：COMPLETE，完整材料 semantic sidecar 与 12D full closure；
