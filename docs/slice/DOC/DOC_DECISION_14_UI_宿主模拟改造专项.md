...
| 里程碑 | 含义 | UI 侧动作 |
|---|---|---|
| M-MVP 之前 | DLL/Worker 尚不可端到端调用 | **主干 UI 布局与功能保持原样，不做任何改造** |
| M-MVP 达成 | 能力包最小闭环可用 | 14E 启动，新建 `slicer_ui_host_sim` |
...
**目标（B）**：UI 层迭代时新增 3D 视角（相机）操作，对齐业界常用切片/打印软件的模型操作体验，
并在设置层提供**俯视 ⇄ 3D** 切换。
...
3D 视角新增的全部是【呈现层 + 拾取层】：
  ...
  · 碰撞 / 越界               → 仍由 geometry.collision 与 scene.apply_operation 给出
```
...
### 6.4 🔴 能力面缺口：`scene.get_viewdata` 的网格 DTO 从未定义（A · 重要发现）

现有描述只把三角数据列为**可选**，且从未给出字段级规格：

| 出处 | 原文 |
|---|---|
| `CLAUDE_13` 能力表 | `scene.viewdata` = 「俯视轮廓、bbox、**可选三角缓冲**」 |
| `INT_16` 示例 | `"viewdataIdentity": "vd:43:inst-01:lod0"` —— 出现 `lod0`，但 LOD 语义未定义 |
| 现有实现 | `SceneViewGeometry` 面向**俯视投影**构建 |

**风险**：`14A-04`（能力 DTO 字段级规格）是**契约冻结**任务。若冻结时不把网格 DTO 定死，
等 UI 做 3D 才发现字段不够，就要修改**已交付给打印侧的 ABI** —— 那时改动成本极高。

**要求（本专项对 14A-04 的输入）**：即使首版实现只返回俯视轮廓，
`viewdata` 的 DTO 也**必须预留并定义**以下语义，避免 ABI 二次破坏：

```text
① 网格缓冲     顶点 / 法线 / 索引的布局、字节序、单位（mm）
② LOD 分级     lod0..N 的含义、由谁决定、宿主如何请求指定 LOD
③ 实例变换     网格是"已应用世界变换"还是"局部坐标 + 变换矩阵"（建议后者，便于宿主复用）
④ 传输策略     大模型网格可达数十 MB，跨 ABI 如何传递（分块 / 共享内存 / 缓冲三态协议）
⑤ 缓存与失效   viewdataIdentity 的构成规则与失效条件（模型/变换/LOD 变化时如何判定过期）
```

> 第 ④ 条尤其要早定：现有缓冲三态协议（`out_required`）是为中小结果设计的，
> 一次性返回几十 MB 网格是否合适，需要在 14A-04 就有结论，而不是等 UI 撞上去。
...
| 权威求值 | 复用既有，**不新增** |
| 对 14A-04 的约束 | 网格 DTO 必须在契约冻结时定义（§6.4）→ 规格已成文：`DOC_SCHEMA_14_SceneViewData网格DTO规格.md` |
...
| 关联 | 说明 |
|---|---|
| **M-MVP（14C-06 + 14D-05）** | 本专项全部任务的前置（§2.4）|
| **14A-04** | 必须按 §6.4 定义 viewdata 网格 DTO，否则 3D 视角会迫使 ABI 二次变更 |
...
| **UI-R4** | **网格 DTO 未在 14A-04 定义 → 3D 迫使已交付 ABI 二次变更** | **高** | 🟢 **已缓解**：规格已成文 `DOC_SCHEMA_14_SceneViewData网格DTO规格.md`，列为 14A-04 冻结输入 |
| **UI-R5** | 3D 渲染引入 Qt3D / OpenGL 依赖，与"参考实现可移植"目标冲突 | 中 | 相机与交互逻辑（`CameraController` / `ViewModeSwitch`）与渲染后端解耦；可移植清单只承诺前者 |
