...
### 6.0 🚧 渲染后端选型已移出本专项（2026-08-06）

```text
独立专项：DOC_DECISION_RENDER_模型显示后端选型与渲染接口冻结.md
本专项与 14E【不选后端】，只面向已冻结的 IRenderBackend 接口编程。
```
...
| 约束 | 说明 |
|---|---|
| **不得直接调用图形 API** | `SceneRenderPolicy` 必须经 `IRenderBackend`；出现 GL/D3D/Qt 图形类型即为越界 |
| **最低后端要求** | 只需 `cpu_raster` 即可通过 14E 验收；GPU 后端是可选加速，不是前置 |

> UI-M7（相机期跨 DLL 调用恒为 0）对应渲染专项的冻结条款 **R-C8**，两处必须同时满足。
> 越界高亮等**判定结果由模块给出**（R-C5），后端与 UI 都只负责绘制，不得自行判定。
...
| 原点/轴向 | `buildVolume.origin/xDirection/yDirection` | 同左；不得由相机方向反推设备坐标 |
| 小格 / 大格 | 1 mm / 10 mm | 1 mm / 10 mm |
| 自适应 | `<4 px/mm` 隐藏小格，大格常显；`≥2 px/mm` 显示刻度标签 | 同左，并启用深度测试 |

`230 × 100 × 60 mm` 只是产品 fallback，不得写死在渲染器；设备 Profile 可覆盖。原点或轴向
未解析时只能显示带“未解析”诊断的产品 fallback，不得冒充生产设备真值。网格仅为显示辅助，
