...
> 状态：ACTIVE / 14A-07 COMPLETE / R-B-02 CONTROLLED UPDATE
> 日期：2026-08-10
> 范围：Stage 14 首版能力包的 assimp、miniz、LibTIFF、meshoptimizer

## 1. 审查结论

四项依赖均采用允许商业二进制再分发的宽松许可。首版可分发，但发布包必须包含根目录
...
| meshoptimizer 1.1 | R-B-02 通过 `x64-windows-static-md` 静态链接进 `slicer_base`，第三方类型不越过内部 adapter | MIT | 所有能力包和 Qt Runtime 携带 NOTICE 与 `licenses/meshoptimizer.txt`，不分发额外 DLL |
...
meshoptimizer  vcpkg_installed/x64-windows-static-md/share/meshoptimizer/copyright
```
...
R-B-02 的 meshoptimizer 接入由用户于 2026-08-10 明确授权。该受控更新不授权启用 Assimp、
切换默认 TIFF backend、修改 RGBWSV 协议或引入动态 meshoptimizer runtime。

## 6. 修订记录

| 日期 | 版本 | 内容 |
|---|---|---|
| 2026-08-10 | v1.1 | R-B-02 增加 meshoptimizer 1.1：冻结 MIT 文本、静态 `x64-windows-static-md` 链接与无额外 DLL 分发边界。 |
| 2026-08-05 | v1.0 | 14A-07 首版审查。 |
