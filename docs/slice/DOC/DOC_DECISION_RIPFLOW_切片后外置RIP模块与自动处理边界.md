...
## 2026-09-09 D-08 授权补充（优先于下方历史资产描述）

本次固定使用 `rip_project/RIPDLL_20260909` 全套资源，原版存放于 `rip_project/RIPDLL`。
后续供应方按 `RIPDLL_YYYYMMDD` 投放，工程显式选择并记录版本来源，不能自动加载日期最新目录。
新增 `ripMode`：正常 RIP=0、3 倍墨量 RIP=1；新配置默认 0，已有 v1/v2 配置迁移到 1。
供应方 0 为直通、1 为完整流程，名称不承诺每通道乘 3；0 跳过补光油、肤色特殊处理及白墨挂网。
输出验证只更名：单色=`strict_s2`，彩色=`diagnostic_unvalidated`。现有阈值、诊断隔离目录与
`s2PublicationEligible=false` 保持不变，不将界面业务名称当作打印验收结论。
RIP 设置与结果合同以新增 v3 承载 ripMode，旧 schema 不重写，切片生产协议不变。
