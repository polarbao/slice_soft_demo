...
> 阶段：Stage 15 ｜ 状态：**COMPLETE / PRODUCTION ENABLED** ｜ 版本：v1.9 ｜ 更新：2026-08-04
...
| 15D | 验收与证据产出 | ✅ 15D-01..05 完成 |
| 15E | 交付收口 | ✅ 15E-01..03 完成；生产 Profile 已启用 |
...
| G7 | **实物打样确认** | ✅ 用户在取得候选/对照包与软件证据后明确授权放行；Agent 未直接观察实物，仓库未附照片 |
| G8 | 注册表翻转与文档同步 | ✅ enabled/production 与文档同步完成 |
...
上述证据证明 Legacy 目标路径能够仅在命中纯白纹理像素时写 W，并保持两份报告的逐层/汇总一致；G1、G2、G3、G4、G5 已由统一 Gate 自动关闭。历史 Golden 与 Stage 10 输出契约使用运行时配置固定源姿态，避免产品自动定向和 Z 落台默认值改变测试坐标语义。G7 的放行来源为用户在取得候选/对照包及软件证据后的明确收口授权；Agent 未直接操作或观察打印设备，仓库未附实物照片。随后 G8 完成生产 Profile 翻转。
...
| 编号 | 风险 | 等级 | 状态 |
|---|---|---|---|
| R1 | 白墨工艺特性差异 | **高** | 🟢 已由 G7 放行；仓库保留“无实物照片附件”的证据边界 |
...
Stage 15 的 19 张任务卡全部完成。`textured_nail_rgb_white_ondemand_lower_support`
已作为普通生产 Profile 启用；Stage 14 外部 RIP 集成和 12G-TCWS 同包透明/不透明复用仍是
独立问题，不因 Stage 15 收口自动完成或解冻。
...
### 2026-08-04 · 15E-03 Stage 14 Q2 证据回填

- `DOC_ANALYSIS_14_Q2` 与 `DOC_CHECKLIST_14` 已补充 F-01/F-02/F-04、strict Reader、Golden 和 Quick CI 项目内证据。
- 当前只证明显式不透明白 Profile 可用普通 W 通道承载，不证明外部目标 RIP 或实物工艺通过。
- 原 Q2/Q3 继续保留，12G-TCWS 的同包透明/不透明复用问题继续冻结。

### 2026-08-04 · 15D-05 / 15E-02 放行收口

- 软件侧全量 Gate 在提交 `3746444` 上重跑通过：G1..G5、零漂移、性能、相关单测与 Qt self-test 均 PASS。
- 已生成 F-01 按需补白包和白墨填充对照包；用户随后明确授权完成 15E-02，作为本次非自动化放行来源。
- 场景注册已翻转为 `enabled: true` / `productionSafety: production`，普通 UI Profile 列表可见。
- 证据边界：Agent 未直接执行或目视实物打印，仓库未提交照片；不得把本次放行扩展解释为外部 RIP 或 12G 已验收。
