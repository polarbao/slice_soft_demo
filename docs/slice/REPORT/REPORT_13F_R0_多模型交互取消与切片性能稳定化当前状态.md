...
五个资产的只读顶点统计均显示相同的绝对 Z 偏移问题：

| 模型 | 源 Z 范围 mm | 物理高度 mm | 旧层数估算 | 落台后层数 |
|---|---:|---:|---:|---:|
| `segment_101` | `303.7308..310.7060` | `6.9752` | `8177` | `184` |
| `segment_102` | `305.4855..312.1288` | `6.6433` | `8214` | `175` |
| `segment_103` | `304.3297..309.8895` | `5.5598` | `8155` | `147` |
| `segment_104` | `304.3556..309.3974` | `5.0418` | `8143` | `133` |
| `segment_105` | `305.8059..309.8463` | `4.0404` | `8154` | `107` |

该表只读取 OBJ 顶点，没有对其余四个模型执行切片。
...
| 项目 | 仅落台基线 | 当前纵向基线 |
|---|---:|---:|
| 有效包围盒 | `Z=0..6.9752 mm` | `Z=0..6.9752 mm` |
| Grid | `650 x 286 x 184` | `303 x 614 x 184` |
| 核心计算 | `1348.5702 ms` | `2366.3419 ms` |
| 峰值工作集 | `285294592 bytes` | `285462528 bytes` |
| 完整写包总耗时 | `3719.056 ms` | `6516.322 ms` |
| TIFF 保存 | `820.021 ms` | `1309.887 ms` |
| Preview 保存 | `564.010 ms` | `863.927 ms` |
| TIFF 数量 | `184` | `184` |
| RIP Reader | `PASS` | `PASS` |

“仅落台基线”是 13E-R1 平面朝向修订前的诊断证据；当前产品行为以“当前纵向基线”
为准。X/Y DPI 分别为 635/600，旋转后物理采样轴互换，因此两个 Raster 不能按
逐像素完全相同解释。两条路径都已消除八千层异常，本次根因仍与用户排版产生的 XY
平移无关。
