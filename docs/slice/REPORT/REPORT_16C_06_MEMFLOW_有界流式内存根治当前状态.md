...
**已落地两笔，均为「严格更少的工作量、判据一字不改」：**

```text
MF-13a  每个被校验的层被读两次、IFD 解析两次   -> 隔离口径 -34.5%
MF-13b  逐字节七件事改为逐通道 256 桶直方图     -> 隔离口径 -31.7%
        两者合计 11,482 -> 5,272 ms（-54.1%），CPU 时间同幅
MF-14a  逐像素热路径去掉约 210 亿次边界检查     -> compose 窗口 -19.0%
```

**生产口径端到端（MF-13 部分，机器较空、区间不重叠）：**

```text
packagePublishMs  46,806 -> 22,042   (-52.9%)
totalMs           96,754 -> 67,896   (-29.8%)
```

MF-14a 之后应再降，但其耗时数字**测于 4~5 个 MSBuild 并行构建期间，
只可信方向不可引用幅度**，待空窗复测。
