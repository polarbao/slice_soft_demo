...
> ⚠ **口径警告（2026-09-08 追加）：本文档全部耗时数字都是 CLI 口径。**
> 后续 MF-12 查明 **CLI 与生产不是同一条代码路径** —— `LegacySceneLayerAdapter`
> 把 `write_reports` 写死为 false，故材料闭合精确分析在生产路径（DLL→Worker）上
> **从不执行**。本文 §5.2 的验收口径、§5.7 的耗时归因、以及 18.9 分钟 /
> 1,135,091 ms 这类数字，**都只对 CLI 成立，不可用于生产路径**。
> 生产口径的权威数字见 `REPORT_16C_06_MEMFLOW_有界流式内存根治当前状态.md`
> 的 §0.0，那里还列了本专项已作废的数字。

> 文档状态：**用户两项原始阻塞均已实测解除（见 5.11）**
> 版本：v1.16 ｜ 日期：2026-09-08
...
### 5.10.2 场景路径（UI 多实例）：已完成（2026-09-06）

| 指标 | 专项介入前 | 现在 | 目标 |
|---|---|---|---|
| 单实例斜率 | 73.7 MB/层 | **0（与层数无关）** | ✅ |
| 单实例 15/29/58 层 | 2.12/3.15/5.29 GB | **1.019 GB（三档相同）** | ✅ |
| 双实例 15 层 | 3.84 GB | **2.02 GB** | ✅ |
| 三实例 58 层 | （斜率外推 &gt;11 GB） | **2.98 GB** | ✅ |

实例侧、合成侧、写入侧三处均已流式（`7d7a627`）。**峰值只随实例数与列数变化，
与层数无关**，全量回归 10 失败 / 229 与既有清单一致。

耗时侧本轮再加 MF-03X5（通道统计稀疏化，空列份额解析补齐）：
`a-2/0.2.obj @0.1mm` 的 `layerComputeMs` 由 45,402 降至 35,100（min-of-3，-22.7%）。
至此单模型侧三处按幅面固定开销（X3 内部空腔缓冲、X4 compose 输出缓冲、
X5 通道统计）全部处理完，剩下的 TIFF 写出是输出本身、不可省。
...
剩余项要么收益有限、要么属另一类改动。
~~**场景路径的内存优化尚未完成**~~ —— **已于 2026-09-06 完成并实测验收，见 5.11。**
（写于 2026-09-05 的原判断保留于此，作为「合成与写入侧还差一步」这一预判的对照。）

---
### 5.11 用户原始阻塞的最终验收（2026-09-06）

本专项由用户的两项真实生产阻塞发起。此处给出**两项的实测终值**，
数据均来自 `stage16c06_scene_memory_bench`（`peakWorkingSetBytes` 取进程峰值工作集）。

#### 5.11.1 阻塞一：`a-2/0.2.obj` @10um 耗时约 20 分钟

| 指标 | 专项介入前 | 现在 | 变化 |
|---|---|---|---|
| 耗时 | 18.9 分钟 | 9.44 分钟 | **-50.0%** |
| 峰值 | 22~34 GB | 0.84 GiB | **-97% 以上** |

#### 5.11.2 阻塞二：`0.2.obj` + `0.3.obj` 同场景 @10um 内存不足、切片失败

**这是本专项的最终验收项。**

```text
BENCH layerHeightMm=0.01 models=2 instances=2 valid=1
      elapsedMs=1.8755e+06 peakWorkingSetBytes=1905709056
```

| 指标 | 专项介入前 | 现在（实测） |
|---|---|---|
| 结果 | **内存不足，切片失败** | `valid=1`，正常出包 |
| 峰值 | 外推约 270 GB（物理内存 31.6 GB） | **1.91 GB**（1.775 GiB） |
| 耗时 | 无法完成 | 31.3 分钟 |

外推值的来历见 §5.8：改前场景路径是三份整栈同时驻留
（N 份每实例栅格 10 B/列/层 + 一份合成结果 6 B/列/层），
按用户双模型 10um 的列数与层数合计约 269.6 GiB —— 是物理内存的 8.5 倍，
故当时不是「慢」而是必然失败。现在峰值只随实例数与列数走，与层数无关。

#### 5.11.3 这两项数字各自出自哪一步

| 改动 | 解决的是 |
|---|---|
| MF-03X1 / X2a | 单模型七个 mask 整栈 31.11 GB（阻塞一的内存） |
| MF-03X3 / X4 / X5 | 单模型三处按幅面固定开销（阻塞一的耗时，18.9 -> 9.44 分钟） |
| MF-05 | 场景三份整栈（阻塞二的全部收益） |

分工是清楚的：**阻塞一的收益全在单模型侧，阻塞二的收益全在 MF-05。**
两者互不替代 —— 这也是为何 §5.6 首次归因（把双模型失败算到三个 mask 栈上）是错的。

#### 5.11.4 验收口径的诚实边界

三点需要写明，避免这组数字被读得比它实际支持的更强：

1. **31.3 分钟没有改前对照。** 改前那次是失败的，不存在可比耗时。
   这个数只能说明「现在能跑完，且量级合理」，不能说明耗时改善了多少倍。
2. **峰值取的是进程工作集**，含分配器未归还给 OS 的部分，
   故它是上界而非「实际需要」的下界，用于判断「会不会 OOM」是合适的。
3. **本次是单次运行**，未做 min-of-N。对耗时而言不足以下精确结论
   （本机短跑波动可达 47%），但对峰值而言，1.91 GB 与 270 GB 之间
   隔着两个数量级，波动不构成威胁。

---

### 5.12 MF-03X2b：准入放开的范围，与 shape 档为何暂不放开

X2a 的准入门一次挡下四类配置。X2b 逐档评估，本节给出结论与依据。

#### 5.12.1 已放开：full vertical projection

它与 bottom projection **同构**：同为按列的半开区间 `[0, upperBound)`，
同样只依赖本层 model mask，差别只有两处 —— 上界由按列标量
`support_source_layers` 换成列区间的 `upperLayer`，类型换成
`FullVerticalProjection`。

retained 那边要先对整栈做一次归约（`compute_last_model_layers`）才拿得到上界；
有界路径**不需要这次归约，因为列区间本身就携带了它的结果**。这正是本档能
放开的全部依据，故把 retained 的归约函数下沉到与 `BoundedSpanLastModelLayer`
同一文件并置 —— 两份实现同屏可见，改一份而不改另一份会立刻显眼。

**顺带修掉一处死代码。** 原准入里有一条
`if (mode == "full_vertical_projection") return Reject(...)`，它**永不可达**：
`full_vertical_projection` 不含 `bottom_projection`，在它之前的
`!ModeIncludesBottomProjection` 已经先把它拒成
`support_mode_without_bottom_projection` 了。本次把 full vertical 的判定移到
最前，其余两条的先后**刻意保持原样**，以免改掉既有的拒绝理由。

**一处必须先更正的上游前提。** 三档谓词都不可以直接读 `upperLayer`：
采样在 `startLayer > endLayer` 时 `continue`，而 `has_model` 已在此之前置真
（`slicer.cpp` 的 `sample_relief_heightfield_masks`），故确实存在
`hasModel == true` 但 `lower/upper` 仍为 -1 的列。已抽出归一化守卫
`BoundedSpanLastModelLayer` 并加了一项 40 组随机夹具的比对，
证明它与整栈归约**逐列同值**。

#### 5.12.1.1 full vertical 档的端到端验证，与一个必须写明的反直觉发现

四判据（r01 / gb04 / tm2-5 / yz）**全部是 bottom 档**，它们逐字节全等只证明
「没有破坏旧档」，**不足以验收新档**。故补做一次专门的对照：

**方法。** 同一份配置（`support.mode = "full_vertical_projection"`）跑两次，
bounded 侧用当前 exe，retained 侧用「把准入里那两处 placement 赋值临时改回
`Reject`」后重建的 exe。两份配置逐字节相同、只有输出目录不同，
**唯一变量就是代码路径本身**。资产用 r01（94 层）与 gubao04（129 层，六材质）。

**结果：TIFF 逐字节全等，17 份报告逐字段全等**（含 `supportTypeStats`：
`full_vertical_projection: 131956` / `bottom_projection: 0`，
`lowerEnabled: false` / `fullVerticalProjectionEnabled: true`）。

**发现：在 `relief_heightfield` 下，full vertical 与 bottom 的支撑像素恒等。**

起因是对照跑出的 TIFF digest 与 bottom 档的长期基线**完全相同**
（r01 `3cbfdec…`、gubao04 `8315b63c…`），第一眼像是档位根本没生效。
不是 —— 原因是该采样模式下**每列恰好是一个连续闭区间**：

```text
bottom        [0, lowerLayer)        中无模型的列 -> 支撑
full vertical [0, lastModelLayer)    中无模型的列 -> 支撑
```

`[lowerLayer, upperLayer]` 全部有模型（闭区间无空洞），故第二个区间比第一个
多出来的那段 `[lowerLayer, lastModelLayer)` **一列也不会命中**。两者的支撑像素
集合因此恒等，**只有 `SupportType` 标签不同**，而标签不进 TIFF。

两条推论要记住：

1. **TIFF 比对对这两档没有区分力。** 只看层哈希会把「档位生效且正确」与
   「档位完全没生效」判成同一结果。验收必须比报告 —— 这正是本次比了
   全部 17 份报告而非只比 `package_report.json` 的原因。
2. **本档的收益不在输出，而在路径。** 输出本就与 bottom 相同；放开它的意义是
   这一档**不再退回 retained 整栈**，即拿到 X2a 的全部内存收益。
   若哪天采样模式变成允许一列多个区间（例如 layer_slab 候选），
   两档就会真正分岔，届时这条恒等结论**立即失效**。

#### 5.12.2 暂不放开：shape 档（R-8 成立，且比预判更严重）

放开 `shape_enabled` 前，先对「形状膨胀可能越出 activeColumns」这一风险
（代号 R-8）做了一次只读静态查证。**结论：成立，且三处比原判断更糟。**

**一、有邻域写入的不止膨胀一项，是四项。**
`OptimizeSupportShape` 的九步里，膨胀、简化闭运算、水平桥接、垂直桥接
都会把「原本为空的列」写成非空。最小面积剔除、模型优先清零、超比例回滚
只做清零，连通域分析只读。

**二、判据本身要更正。** 原先设想「只读邻域再写回本列不构成风险」——
**不成立**。所有加法都经同一守卫 `CanWriteSupportPixel`
（`modelMask[i] == 0 && supportMask[i] == 0`），即**加法的目标集合恰好就是
「模型空且支撑空」的列**，与 R-8 关心的集合完全重合。闭运算写的是本列，
但它进入该分支的前提正是本列原本为空 —— 一样构成风险。

**三、后果不是「支撑少一圈」，是三重且会互相掩盖。**

```text
输出侧  compose_layer 只遍历活动列  -> 非活动列的支撑像素永不进入输出（少）
缓冲侧  逐层重置也只重置活动列      -> 写到非活动列的 1 没有任何一层会清回 0
统计侧  AccumulateSupportLayerStats -> 遍历整幅面，残留像素被每层重复计入（多）
        遍历的是 pixel_count
```

一边少一边多，且「多」的那侧随层数线性放大。对拍时的症状会是
「数字对不上但看不出规律」—— 这类症状最耗排查时间。

**四、累计作用半径没有上界。** 算子串行、后者作用在前者结果上，故保守上界为
`xy_dilation_px + closing_radius_px + 2 * bridge_gap_px`。配置侧只校验
这三个字段 `>= 0`，全仓无任何 clamp。附带发现：`DilateSupport` 是
O(w·h·(2r+1)²) 的朴素四重循环，10um 幅面配 `xyDilationPx = 8` 就是约
2.1e9 次内层迭代/层 —— 这是与 R-8 无关的另一条无护栏风险。

**决策：shape 档本轮不放开。** 一旦放开，必须同时选定剪枝策略：

| 方案 | 代价 | 评价 |
|---|---|---|
| (a) shape 档一律不剪枝（`activeColumns` 传 `nullptr`） | 放弃 X2b 的稀疏收益，保留 X2a 全部收益；每层多付整幅面填充（带宽，非峰值） | **推荐**。三处消费点都已支持 `nullptr`，改动约等于一处三目表达式，且无需任何等价性论证 —— 非剪枝路径就是 retained 语义本身 |
| (b) 按累计半径把活动列外扩一圈 | 一次性 O(w·h)（可分离滑窗），额外 7.4 MB 位图 | 可行但**耦合无强制同步**：任何人加一个新算子或改执行顺序，半径公式就静默失效，症状还是不报错的支撑缺失。若走此路必须配断言把耦合钉住 |
| (c) 自适应/迭代扩张 | — | **不推荐**。把静态可离线论证的集合变成依赖运行时结果的集合，等价性再也说不清 |

#### 5.12.3 两条上游断言的更正

查证同时推翻了我方此前记下的两条：

**其一，「需新抽 `OptimizeSupportShapeSingleLayer`」—— 不必。**
仓库里已有一份真正逐层的实现：`BoundedSupportShapeScanner::ConsumeLayer`
（`support/BoundedSupportShapeScan.{h,cpp}`），`layerIndex` 是真参数、
scratch 跨层复用、自带类型同步与 SHA-256 重放校验，已有测试，
当前标注为「非生产 MF-03B3 扫描」且在 `slicer.cpp` 零引用。把它转正的成本
很可能低于新写一份。**新写会产生第三份需要与 `OptimizeSupportShape`
逐字同步的算法副本** —— 现已有两份（`SupportShapeOptimizer.cpp` 与
`BoundedSupportShapeScan.cpp`，手工同构复制），那本身已是一处同步债。

**其二，「`OptimizeSupportShapeForLayer` 是假单层」—— 事实对，理由要改。**
`layer_index` 恒为 0 属实（它把单层包成一元 vector，循环上界恒为 1）。
但「不能用于逐层调用」的真正理由**不是标签**，而是它每次调用要做
**两次整幅面拷贝**（拷入 modelMask 与 supportMask，再 move 回来），
加上优化器内部的 `originalMask` 与 `addedMask` —— 逐层调用即每层 4 次
`w*h` 分配。**那恰恰是本专项要消除的东西**，比标签硬得多。

#### 5.12.4 其余两档的状态

`base_projection` 与 `outer_varnish` 本轮未评估，仍保持 Reject。
两者的共性是会引入新的整栈（`outerVarnishMasks` / `upperBoundaryMasks`），
属于「要先有有界替代品」而非「准入判据放开即可」，与 shape 档不同类。
...
| 日期 | 版本 | 变更 |
|---|---|---|
| 2026-09-06 | v1.15 | 新增 §5.12.1.1：补做 full vertical 档的端到端对照（四判据全是 bottom 档，只能证明没破坏旧档，不足以验收新档）。方法是同一配置跑两次、retained 侧用「把准入临时改回 Reject」后重建的 exe，唯一变量是代码路径。结果 TIFF 逐字节 + 17 份报告逐字段全等。同时记下一个反直觉发现：`relief_heightfield` 下每列恰好是一个连续闭区间，故 full vertical 比 bottom 多出的那段区间一列也不会命中，两档支撑像素**恒等**、只有 SupportType 标签不同，而标签不进 TIFF —— 对照跑出的 digest 与 bottom 长期基线完全相同正是此故，第一眼像档位没生效。两条推论：TIFF 比对对这两档无区分力，验收必须比报告；本档收益不在输出而在「不再退回 retained 整栈」。若采样模式将来允许一列多区间，该恒等结论立即失效。 |
| 2026-09-06 | v1.14 | 新增 §5.12：MF-03X2b 准入放开。放开 full vertical projection（与 bottom projection 同构，上界由按列标量换成列区间的 upperLayer；retained 需要的整栈归约在有界路径不需要，因为列区间已携带其结果），并把 retained 的归约函数下沉与有界版并置。顺带删掉一条永不可达的 Reject 分支（full_vertical_projection 不含 bottom_projection，会被前一条先拒掉），其余拒绝理由的先后刻意保持原样。先更正一处上游前提：`hasModel` 为真但区间为 -1 的列真实存在（采样的 continue 分支在 has_model 置真之后），抽出归一化守卫 `BoundedSpanLastModelLayer` 并证其与整栈归约逐列同值。shape 档经只读静态查证后**决定暂不放开**：R-8 成立且三处比预判更严重 —— 有邻域写入的是四项运算而非只有膨胀；「只读邻域写回本列不构成风险」这条判据本身错误（所有加法都经同一守卫，目标集合恰是空列）；后果是输出少一圈 + 缓冲永久残留 + 统计逐层放大，一少一多会互相掩盖成「数字对不上但看不出规律」。累计半径无上界校验。同时更正两条上游断言：不必新抽单层优化器（已有 BoundedSupportShapeScanner），且「假单层」的真正否决理由是每层四次整幅面拷贝而非报告标签。 |
| 2026-09-06 | v1.13 | 新增 §5.11：用户两项原始阻塞的最终验收。阻塞二（`0.2.obj`+`0.3.obj` @10um 内存不足）实测 `valid=1`、峰值 1.91 GB、耗时 31.3 分钟，对照改前外推 270 GB（物理内存 31.6 GB，故当时是必然失败而非慢）。同时标注验收口径的三点边界：31.3 分钟无改前对照（改前那次是失败的）、峰值取的是含分配器未归还部分的工作集（是上界）、本次为单次运行未做 min-of-N（对耗时不足以下精确结论，对峰值因隔两个数量级而不受影响）。并按改动归属拆开两项阻塞的收益来源，指出二者互不替代 —— 这正是 §5.6 首次归因出错的地方。据此把 §5.10.4「场景路径尚未完成」标为已完成，原文保留作预判对照。为跑此项，`SceneMemoryBench` 扩展为支持多个不同模型（每模型一个 profile 与 ModelSource、实例轮转取用、幅面按所有模型最大尺寸铺开）。 |
| 2026-09-06 | v1.12 | MF-05 完成：实例侧/合成侧/写入侧三处均已流式，场景峰值与层数彻底脱钩（单实例 15/29/58 层同为 1.019 GB、三实例 58 层 2.98 GB），全量回归 10 失败 / 229 无新增。耗时侧新增 MF-03X5 通道统计稀疏化（45,402 -> 35,100 ms，min-of-3）。5.10.2 由「进行中」改为「已完成」。 |
