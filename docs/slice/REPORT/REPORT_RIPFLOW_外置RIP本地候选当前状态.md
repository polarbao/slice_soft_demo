...
- 参考宿主增加独立 RIP 设置页、QSettings v2、手动入口、自动开关、取消、超时和双终态；
- 新版 `--transparent 0..4` 按 `透明/不透/肤色/白色30/白色50` 五档显式配置并无损传参；
...
| 旧版 fail-closed 基线 | D-01 时 `follow_manifest` 缺字段、opaque/grayBits 超限、tiled/坏布局均拒绝；二值模式已由 D-07 五档模型取代，输出 Gate 继续保留 |
...
| 新版五档适配 | 模块 `1.1.0`，合同 v2；Release `ALL_BUILD`、RIP 定向 CTest 7/7、合同 `positive=4 negative=6` PASS；真实 3 层 Package 的 0..4 五档均 `exitCode=0` 且 3/3 输出，宿主模式 4 诊断结果保持 `transparentMode=4` |

2026-08-25 的新版收口复验通过合同门禁、Release `ALL_BUILD` 和 RIP 定向 CTest 7/7。默认
Runtime 的覆盖部署被部署脚本拒绝，因为用户仍在运行该目录中的 `slicer_ui_host_sim.exe`
（当次 PID 31388）；未强制终止该进程。新版已完整部署到
`runtime/slicesoft-rip-v1.1/Release`，模块自检及 EXE/DLL/tiff 三个源文件 hash 一致性 PASS。

定向构建使用 VS 2026 x64 编译器、Qt 5.15.2、Release Visual Studio、LibTIFF 4.7.1，目标
...
新版 RIP 的 0..4 五档均可执行，但“可执行”不代表“可严格发布”。当前 CLI 没有 grayBits 参数，
grayBits 设置只做输出准入；`--colormode` 仍固定为 0。DPI 不是 CLI 或 DLL 的输入参数；二进制
写入的 600 x 600 TIFF 标签保留为外部元数据，不再限制 Package DPI。各模式是否可发布仍由
每次真实输出的结构、尺寸和 W/S/V Gate 决定。这些失败是严格 `rip` 的预期保护，不得绕过检查发布。操作员可显式选择
...
2. 五档颜色模式的目标工艺效果、`colormode`、W/S/V 极性和设备 grayBits 的打印侧书面合同；
