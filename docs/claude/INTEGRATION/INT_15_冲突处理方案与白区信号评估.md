...
>
> ⛔ **2026-08-04 部分作废：本文「白区信号」一节的结论已失效。**
> 当时推荐的**方案甲「语义 sidecar 掩膜」（逐层 1-bit sidecar）已被用户否决**，
> 且 RIP 侧已选定**路径 D** —— 废弃 `WSV=000` 哨兵，改用 `W=0` 真实材料语义。
> **本文所有关于 sidecar / `WSV=000` / 白区信号方案的"推荐"表述一律不得实施。**
> 权威条款见 `docs/slice/DOC/DOC_DECISION_14_S2_RIP接口合同定案.md`（§1.3 与 §4 作废清单）。
> 本文其余内容（三层功能定义、内部契约、Worker 替换、UI 与长文件拆分）**仍然有效**。
