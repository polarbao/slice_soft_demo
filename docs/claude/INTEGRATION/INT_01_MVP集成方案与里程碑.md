...
>
> ⚠️ **交付时点已修订（2026-08-02）**：本篇的 M0–M5 仍然有效，但**多模型能力的交付时点后移**——`INT_12` 把封装交付拆为 v0.1（W6，单模型全链路）/ v1.0（W18，多模型+预览+取消）/ v2.0，**多模型属 v1.0**。
> 另需注意分工澄清：「MVP 含多模型排版」指的是**打印软件的用户体验**（能导入多个模型并摆放）；而方案 C 的「不提供 `scene.layout`」指的是**切片包不提供 packing 引擎**——宿主做摆放决策与手感，切片包提供 `geometry.collision` 真值。二者不矛盾。裁定见 [`INT_13` §4](INT_13_封装层级一致性核对与完善清单.md)，总计划见 [`INT_12`](INT_12_短中长期总体计划与s14吸纳台账.md)。
