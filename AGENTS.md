...
- `03D-01..04` are complete as of 2026-07-31: the handwritten contract/Release baseline, optional LibTIFF 4.7.1 build and Runtime lane, dual Writer interface, LibTIFF stripped/tiled output, stable writer errors, sibling temporary files, and failure cleanup are executable. Standard LibTIFF tiles use dimensions divisible by 16; legacy nonstandard tile fixtures retain the handwritten compatibility path. `03D-05` is READY for the full equivalence/bad-package/shared-Package gate. The default backend remains handwritten and must not switch without the documented GO gate and explicit authorization.
