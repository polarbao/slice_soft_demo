...
- `03D-01..05` are complete as of 2026-07-31: the handwritten contract/Release baseline, optional LibTIFF 4.7.1 build and Runtime lane, dual Writer interface, LibTIFF stripped/tiled output, stable writer errors, sibling temporary files, failure cleanup, decoded-pixel/tag equivalence, strict/bad-package Reader cases, and shared Legacy/Global/scene Package gates are executable. Standard LibTIFF tiles use dimensions divisible by 16; legacy nonstandard tile fixtures retain the handwritten compatibility path. `03D-06` is READY for the Release performance matrix. The default backend remains handwritten and must not switch without the documented GO gate and explicit authorization.
- `12E-09D-01..06` are fully prepared but unimplemented. They separate diagnostic width from production texture controls, define Legacy layer-count versus Global physical-shell semantics, and close single-material relief W/V selection. They follow 03D and precede 12E-10A.
