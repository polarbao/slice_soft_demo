...
11. **新建分支必须遵守 `docs/git/Git 分支与发布规范.md`。** 硬约束五条，细节见该文：
    ...
    b. **功能分支从 `develop/packaged-slicer` 拉、合回 `develop/packaged-slicer`**；集成线在里程碑时合入 `main`。热修复从 `main` 拉，合回 `main` 与 `develop/packaged-slicer` 两侧——漏了后者，下一次集成线进 `main` 会把修复覆盖掉。**不要从 `main` 拉功能分支**，且 **`main` 上永不直接提交**——它只能由集成线快进过来。（`product/packaged-slicer` 已于 2026-09-18 退场：它与 `main` 停在同一提交、职责重合，职责由 `main` 承接；`product/legacy-slicer` 保留不动——它没有下游、不会合进 `main`（2026-08-05 分叉后主线走了 495 条、它走了 2 条，是另一个产品），且那条分支上三档预设一个都没有，叫 `develop/*` 会把发布线错标成集成线。**注意不是因为「被工作树检出」**——实测被工作树检出的分支**可以改名**、工作树会自动跟随，被挡住的是**删除**。）
    c. **三层分支对应三档闸门**（耗时差一个量级，故分层）：功能分支自测跑 `slicesoft-debug-core`（155 项、12~18 秒）；合入 `develop/packaged-slicer` 跑 `slicesoft-debug-fast`（262 项、约 3 分钟）；**合入 `main` 必须跑全量**——`slicesoft-debug-full`（270 项、约 16 分钟）外加**全量重建**（见 8b）与**字节级基线 PASS**（`scripts/CaptureSliceOutputBaseline.py --verify`），并按**失败集合**比对（见 8c）。没有 CI 也没有 PR 评审，验证就是唯一闸门。**一条豁免**：改动完全落在 `*.md` / `docs/` / `analysis/` 之内时，进 `develop/packaged-slicer` 只需核心档；但集成线 → `main` 那一跳**永不豁免**。**失败集合比对时，未归因的新失败挡住合入、已归因的已知抖动项放行但须留痕**；「归因」要有隔离重跑的分布、具体判据、以及为何与本次改动无关三样，缺一即按未归因处理（当前已知抖动项与门槛见规范第三节）。
