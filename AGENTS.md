...
- `03D-01..06` are complete as of 2026-08-03: the handwritten contract/Release baseline, optional LibTIFF 4.7.1 build and Runtime lane, dual Writer interface, stripped/tiled output, stable errors/cleanup, decoded-pixel/tag/RIP/bad-package/shared-Package gates, and the independent-process Release Writer matrix are executable. The 03D-06 result is `GO_OPTIONAL`: compatibility and memory passed, but main-production warm stripped p50 did not meet the 15% improvement gate. Handwritten remains the default; 03D-07 may only close the optional backend unless a future Gate and explicit authorization reopen default switching.
