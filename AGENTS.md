...
- The 2026-07-31 preparation amendment makes `03D-LIBTIFF` the highest-priority next engineering specialty. It prepares a dual handwritten/LibTIFF writer, exact decoded RGBWSV/tag compatibility, runtime deployment, and Release performance gates. The current writer remains handwritten and the default backend must not switch without the documented GO gate and explicit authorization.
