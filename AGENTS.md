...
- `03D-01` is complete as of 2026-07-31: raw TIFF tag/stripped/tiled/padding/error contracts and a handwritten Writer-only Release baseline are now executable. `03D-02` is the next prepared candidate but requires explicit authorization before vcpkg/CMake/Runtime dependency changes. The current writer remains handwritten and the default backend must not switch without the documented GO gate and explicit authorization.
