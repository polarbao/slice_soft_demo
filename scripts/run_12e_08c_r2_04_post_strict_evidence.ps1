param(
  ...
  [string]$Output = "output/benchmarks/12e_08c_r2_04_post_strict/post_strict_summary.json"
)
  ...
  [string]$SourceId)
{
  ...
  if ($exitCode -ne 0)
  {
    throw "12E-08C-R2-04 failed: source=$SourceId exitCode=$exitCode"
  }
}
...
try
{
  ...
  {
    $caseId = $case.caseId
    Write-Host "== 12E-08C-R2-04 post-strict case: $caseId"
    ...
    $firstReportPath = Join-Path $caseRoot "mesh_repair_post_strict_run_1.json"
    $secondReportPath = Join-Path $caseRoot "mesh_repair_post_strict_run_2.json"
    Invoke-R2PostStrict $executable $configPath $firstReportPath $sourceAsset.path
    Invoke-R2PostStrict $executable $configPath $secondReportPath $sourceAsset.path
    ...
    Assert-Equal $first.options.validatePostRepairEvidence $true "$caseId evidence validator option"
    Assert-Equal $first.productionOutputWritten $false "$caseId production output"
    Assert-Equal $first.evidenceValidation.operationSequencePass $true "$caseId operation sequence"
    Assert-Equal $first.evidenceValidation.sourceMappingPass $true "$caseId source mapping"
    Assert-Equal $first.evidenceValidation.vertexMappingPass $true "$caseId vertex mapping"
    Assert-Equal $first.evidenceValidation.generatedMappingPass $true "$caseId generated mapping"
    Assert-Equal $first.evidenceValidation.attributePass $true "$caseId attribute gate"
    Assert-Equal $first.evidenceValidation.hashConsistencyPass ($caseId -eq "three_mf_texture2d_checker") "$caseId hash gate reachability"
    Assert-Equal $first.evidenceValidation.candidateAccepted ($caseId -eq "three_mf_texture2d_checker") "$caseId candidate acceptance"
    if ($caseId -eq "three_mf_texture2d_checker")
    {
      Assert-Equal $first.evidenceValidation.status "passed" "$caseId validation status"
      Assert-Equal $first.evidenceValidation.postStrictComplete $true "$caseId complete strict evidence"
      Assert-Equal $first.evidenceValidation.postStrictPass $true "$caseId post strict"
      Assert-Equal $first.status "strict_pass_no_repair" "$caseId repair status"
    }
    else
    {
      Assert-Equal $first.evidenceValidation.status "blocked_incomplete_post_strict" "$caseId validation status"
      Assert-Equal $first.evidenceValidation.postStrictComplete $false "$caseId incomplete strict evidence"
      Assert-Equal $first.status "manual_repair_required" "$caseId repair status"
    }
    ...
    Assert-Equal $firstStableJson $secondStableJson "$caseId post-strict repeatability"
    $stablePath = Join-Path $caseRoot "mesh_repair_post_strict_stable.json"
    ...
    $caseResults += [pscustomobject][ordered]@{
      ...
      evidenceValidationStatus = $first.evidenceValidation.status
      evidenceValidationPass = $first.evidenceValidation.pass
      candidateAccepted = $first.evidenceValidation.candidateAccepted
      postStrictComplete = $first.evidenceValidation.postStrictComplete
      postStrictPass = $first.evidenceValidation.postStrictPass
  ...
  $summary = [pscustomobject][ordered]@{
    schema = "slicesoft.mesh_repair_post_strict_evidence.12e_08c_r2_04.1"
    ...
    result = [ordered]@{
      ...
      validatorPassedCases = @($caseResults | Where-Object { $_.evidenceValidationPass }).Count
      nextTask = "12E-08C-R3-01"
    }
  }
  Write-Utf8NoBom $outputPath ($summary | ConvertTo-Json -Depth 64)
  Write-Host "12E-08C-R2-04 post-strict evidence complete: $Output"
}
finally
{
  Pop-Location
